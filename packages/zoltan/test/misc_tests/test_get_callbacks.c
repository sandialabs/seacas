...
void objSizeMultiFn(void *data, int ngid, int nlid, int nids,
                    ZOLTAN_ID_PTR gids, ZOLTAN_ID_PTR lids, int *nbytes,
                    int *ierr) {
  int i;
  *ierr = ZOLTAN_OK;
...
int main(int narg, char **arg) {
  ...
  /* Register callbacks */
  Zoltan_Set_Fn(zz, ZOLTAN_NUM_OBJ_FN_TYPE, (ZOLTAN_VOID_FN *)numObjFn,
                (void *)(&nobj));
  ...
  /* obj_size */
  ierr =
      Zoltan_Get_Fn(zz, ZOLTAN_OBJ_SIZE_MULTI_FN_TYPE, &fn, (void **)&fndata);
  ...
  /* pack */
  ierr =
      Zoltan_Get_Fn(zz, ZOLTAN_PACK_OBJ_MULTI_FN_TYPE, &fn, (void **)&fndata);
  ((ZOLTAN_PACK_OBJ_MULTI_FN *)fn)((void *)fndata, 1, 1, nobj, NULL, NULL, NULL,
                                   NULL, NULL, (char *)&buf, &ierr);
  fndata_val = *fndata;
  ...
  /* unpack */
  ierr =
      Zoltan_Get_Fn(zz, ZOLTAN_UNPACK_OBJ_MULTI_FN_TYPE, &fn, (void **)&fndata);
  ((ZOLTAN_UNPACK_OBJ_MULTI_FN *)fn)((void *)fndata, 1, nobj, NULL, NULL, NULL,
                                     (char *)&buf, &ierr);
  fndata_val = *fndata;
