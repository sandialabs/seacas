...
int PMPI_Init(int *argc, char **argv[]) {
  ...
  if ((_MPI_INIT_STATUS == _MPI_ENDED) || (_MPI_INIT_STATUS == _MPI_STARTED)) {
    ...
    MPI_ERRORS_ARE_FATAL(NULL, &retval);
  ...
  _MPI_COMM_LIST = (_MPI_COMM_IMPL *)_MPI_safeMalloc(
      _MPI_COMM_ARRAY_SIZE * sizeof(_MPI_COMM_IMPL),
      "Error with malloc of COMM_LIST");
  _MPI_DATA_BUFF = (_MPI_DATA_ENTRY *)_MPI_safeMalloc(
      _MPI_DATA_ARRAY_SIZE * sizeof(_MPI_DATA_ENTRY),
      "Error with malloc of DATA_BUFF");
  _MPI_TYPE_LIST = (_MPI_TYPE_DES *)_MPI_safeMalloc(
      _MPI_TYPE_ARRAY_SIZE * sizeof(_MPI_TYPE_DES),
      "Error with malloc of TYPE_LIST");
  _MPI_OP_LIST =
      (_MPI_OP_TYPE *)_MPI_safeMalloc(_MPI_OP_ARRAY_SIZE * sizeof(_MPI_OP_TYPE),
                                      "Error with malloc of OP_LIST");
  ...
  _MPI_Comm_Insert0(0, MPI_COMM_WORLD,
                    MPI_ERRORS_ARE_FATAL); /* This inserts MPI_COMM_WORLD
                                              as the 1st communicator */
  return MPI_SUCCESS;
}
/*=============================================================================================*/
