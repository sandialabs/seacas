...
class ExodusProperty
{
  public:
    ...
    /** Tells whether the attribute has an invalid type (currently not one of REAL, INTEGER, or STRING)
     *
...
class ExodusPropertyManager
{
  public:
    ...
    double   get_optional(const std::string &attribute_name,
                          double             optional_value) const;
    int64_t  get_optional(const std::string &attribute_name,
...
bool read_exodus_entity_properties(int mesh_exoid, ex_entity_type type, int id, ExodusPropertyManager& attributes);
