...
void *input_nodal_var(void *varg)
{
  ...
  for (i=0; i < num_node; i++) {
    if (!approx_equal(data[i], (arg->timestep-1)*10 + arg->threadid+1 + (float)i / 100.0)) {
      fprintf(stderr, "ERROR: Thread %d: Nodal variable %d mismatch at node %d: Got: %f, expected %f\n",
	      arg->threadid, arg->threadid+1, i+1, data[i], (arg->timestep-1)*10 + arg->threadid+1 + (float)i / 100.0);
    }
  }
  free(data);
  return NULL;
