...
void *input_nodal_var(void *varg)
{
  ...
  if (arg->timestep == 1) {
	...
	fprintf(stderr, "ERROR: Thread %d: X Coordinate mismatch at node %d: Got: %f, expected %f\n",
    ...
    for (i=0; i < segment; i++) {
      if (!approx_equal(data[i], (float)(begin+i) / 100.0)) {
	fprintf(stderr, "ERROR: Thread %d: Y Coordinate mismatch at node %d: Got: %f, expected %f\n",
		arg->threadid, i+1, data[i], (float)(begin+i) / 100.0);
      }
    }
    ...
    for (i=0; i < segment; i++) {
      if (!approx_equal(data[i], (float)(begin+i) / 100.0)) {
	fprintf(stderr, "ERROR: Thread %d: Z Coordinate mismatch at node %d: Got: %f, expected %f\n",
		arg->threadid, i+1, data[i], (float)(begin+i) / 100.0);
      }
    }
  }
  ...
  if (arg->threadid < NUM_NODAL_VAR) {
    ...
    if (strcmp(db_name, ex_name) != 0) {
      fprintf(stderr, "ERROR: Thread %d: Incorrect variable name for variable %d: Got: %s, expected %s\n",
  ...
  for (var = 1; var <= NUM_NODAL_VAR; var++) {
	...
	fprintf(stderr, "ERROR: Thread %d: Nodal variable data mismatch in variable %d at node %d: Got: %f, expected %f\n",
		arg->threadid, var, i+1, data[i], (arg->timestep-1)*10 + var + (float)(begin+i) / 100.0);
      }
    }
  }
  free(data);
