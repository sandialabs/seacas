...
class PartIdMapping
{
 public:
  ...
  : m_idsAssigned(false)
  {
    set_error_handler([] (const std::ostringstream& errmsg) {
 ...
 private:
  ...
  void validate_no_extra_fields()
  {
    ...
    if(!requiredCondition) {
      std::ostringstream errmsg;
      errmsg << "Error!  Each line should not contain more than the following fields (with at least one node):  "
  ...
  void validate_node_count(const Topology& topology, size_t numNodes)
  {
    ...
    if(numNodes != numTopologyNodes) {
      std::ostringstream errmsg;
      errmsg << "Error!  The input line appears to contain "
