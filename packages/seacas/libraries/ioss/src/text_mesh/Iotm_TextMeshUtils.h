...
{
  switch(t) {
  case SPLIT_BY_TOPOLOGIES:    return out << "SPLIT_BY_TOPOLOGIES"; break;
  case SPLIT_BY_ELEMENT_BLOCK: return out << "SPLIT_BY_ELEMENT_BLOCK"; break;
  case SPLIT_BY_DONT_SPLIT:    return out << "SPLIT_BY_DONT_SPLIT"; break;
  default: return out << "SPLIT_INVALID"; break;
  }
...
class EntityGroup
{
 ...
 public:
  ...
  const GroupData* get_group_data(unsigned id) const
  {
    if(is_assigned(id)) {
      ...
      return m_groupDataMap[iter->second];
    }
 ...
 protected:
  ...
  void assign(GroupData &groupData)
  {
    ...
    m_groupDataMap[groupData.name] = &groupData;
  }

  void assign_id_from_standard_name()
  {
    for (GroupData &groupData : m_groupDataVec) {
      if (groupData.has_name()) {
  ...
  mutable bool m_idsAssigned;
  mutable std::unordered_map<std::string, GroupData *> m_groupDataMap;
...
class Nodesets : public EntityGroup<EntityId, NodesetData<EntityId>>
{
 public:
  Nodesets()
  : EntityGroup<EntityId, NodesetData<EntityId>>("NODESET", "NODELIST_", {"BLOCK_", "SURFACE_"}) {}
};
...
public:
  ...
  void split(SidesetData<EntityId, Topology>* const sideset,
             const std::vector<ElementData<EntityId, Topology>>* const elementData)
  {
    m_splitMap.clear();
    m_sideset = sideset;
...
private:
  std::vector<size_t> get_entries_local_to_proc(const std::vector<size_t>& index, int proc) const
  {
    ...
    for(size_t elemPairIndex : index) {
      const std::pair<EntityId,int>& elemSidePair = m_sideset->data[elemPairIndex];
      if(is_element_pair_local_to_proc(elemSidePair, proc)) {
  ...
  bool is_element_pair_local_to_proc(const std::pair<EntityId,int>& elemSidePair, int proc) const
  {
    EntityId elemId = elemSidePair.first;
    auto iter = std::find(m_elementData->begin(), m_elementData->end(), elemId);

    if(iter == m_elementData->end()) {
      std::ostringstream errmsg;
      errmsg << "Sideset with name: " << m_sideset->name << " is referencing an invalid element: " << elemId;
      m_errorHandler(errmsg);
    }

    return iter->proc == proc;
  }
  ...
  using Criterion = std::function<std::string(const SidesetData<EntityId, Topology>& sideset,
                                              const ElementData<EntityId, Topology>& elemData,
                                              int side)>;

  void split_by_criterion(Criterion criterion)
  {
    if(nullptr == m_sideset || nullptr == m_elementData) return;

    size_t index = 0;
    for(const SidesetDataType<EntityId>& elemSidePair : m_sideset->data) {
      EntityId elemId = elemSidePair.first;
      int side = elemSidePair.second;
      auto iter = std::find(m_elementData->begin(), m_elementData->end(), elemId);
      if (iter == m_elementData->end()) {
        std::ostringstream errmsg;
        errmsg << "Error!  Sideset with id: " << m_sideset->id << " and name: " << m_sideset->name
               << " has reference to invalid element '" << elemId << "'.";
        m_errorHandler(errmsg);
      }

      std::string key = criterion(*m_sideset, *iter, side);
  ...
  void split_by_topology()
  {
    Criterion criterion = [](const SidesetData<EntityId, Topology>& sideset,
      ...
      if(sideset.has_standard_name()) {
        return "SURFACE_" + elemData.topology.name() + "_" + elemData.topology.side_topology_name(side) + "_" + std::to_string(sideset.id);
      }
      return sideset.name + "_" + elemData.topology.name() + "_" + elemData.topology.side_topology_name(side);
    };

    split_by_criterion(criterion);
  }
  ...
  void split_by_element_block()
  {
    Criterion criterion = [](const SidesetData<EntityId, Topology>& sideset,
                             const ElementData<EntityId, Topology>& elemData, int side) {
      if(sideset.has_standard_name()) {
        return "SURFACE_" + elemData.partName + "_" + elemData.topology.side_topology_name(side) + "_" + std::to_string(sideset.id);
      }
  ...
  {
    if(nullptr == m_sideset) return;

    std::vector<size_t> splitIndex(m_sideset->data.size());
    std::iota(std::begin(splitIndex), std::end(splitIndex), 0);
    SplitData& splitData = m_splitMap[m_sideset->name];
  ...
  SidesetData<EntityId, Topology> const * m_sideset;
  std::vector<ElementData<EntityId, Topology>> const *  m_elementData;
...
class Sidesets : public EntityGroup<EntityId, SidesetData<EntityId, Topology>>
{
 public:
  ...
  Sidesets()
  : BaseClass("SIDESET", "SURFACE_", {"BLOCK_"}) {}
 ...
 private:
  ...
  void parse_sideset_option(const std::vector<std::string> &sidesetOptionGroup)
  {
    if (sidesetOptionGroup.size() > 1) {
      ...
      SidesetData<EntityId, Topology>* sideset = m_data.sidesets.add_group_data(parser.get_name(), parser.get_sideset_data());
...
class TextMeshParser
{
 ...
 private:
  ...
  void parse_description()
  {
    ...
    auto compareLess = [](const ElementData<EntityId, Topology> &lhs, const ElementData<EntityId, Topology> &rhs) {
