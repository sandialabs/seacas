...
class Assemblies : public EntityGroup<EntityId, AssemblyData<EntityId>>
{
 public:
  ...
  Assemblies() : EntityGroup<EntityId, AssemblyData<EntityId>>("ASSEMBLY", "ASSEMBLY_", {"BLOCK_", "SURFACE_", "NODELIST_"}) {}
...
{
 public:
  Nodesets() : EntityGroup<EntityId, NodesetData<EntityId>>("NODESET", "NODELIST_", {"BLOCK_", "SURFACE_", "ASSEMBLY_"}) {}
};
...
struct SidesetData : public EntityGroupData<EntityId, SidesetDataType<EntityId>> {
  ...
  std::vector<SideBlockInfo> get_side_block_info() const { return sidesetSplitter.get_side_block_info(); }
 ...
 private:
  ...
  void check_name_collision_with_group(const SrcDataGroup& srcGroup, const DestDataGroup& destGroup)
  {
    ...
    for (const auto &srcGroupData : srcGroup.get_group_data()) {
      check_name_collision_with_entity_sets(srcGroupData, destGroup.get_group_type(), groupNames);
    }
  }
  ...
  template<typename T>
  void check_assembly_member_reference_in_group(const AssemblyData<EntityId> &assemblyData, const T& group)
  {
    for(const std::string& entry : assemblyData.data) {
      if(!group.is_registered(entry)) {
        std::ostringstream errmsg;
        errmsg << "Error!  Assembly with id: " << assemblyData.id << " and name: " << assemblyData.name
            << " has reference to invalid " << group.get_group_type() << " '" << entry << "'.";
        m_errorHandler(errmsg);
      }
    }
  }
  ...
  void check_assembly_member_reference()
  {
    for (const AssemblyData<EntityId> &assemblyData : m_data.assemblies.get_group_data()) {
      ...
      default:
        std::ostringstream errmsg;
        errmsg << "Error!  Assembly with id: " << assemblyData.id << " and name: " << assemblyData.name
            << " does not have a valid assembly type '" << assemblyType << "'.";
        m_errorHandler(errmsg);
      }
    }
  }
