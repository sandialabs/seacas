...
unsigned TextMesh::spatial_dimension() const
{
  return m_dimension;
}
...
void TextMesh::parse_dimension_option(const std::vector<std::string> &option)
{
  if (option.size() > 1) {
    ...
    ThrowRequireMsg(m_dimension == 2 || m_dimension == 3, "Error!  Spatial dimension not defined to be 2 or 3!");
  }
}
  ...
  for (size_t i = 1; i < groups.size(); i++) {
    ...
    if (optionType == "coordinates") {
      ...
      fmt::print(Ioss::OUTPUT(),
          ...
          "\tPROC_ID,ELEM_ID,TOPOLOGY,{NODE CONNECTIVITY LIST}[,PART_NAME[,PART_ID]] (specifies element list .. first "
          "argument)\n"
...
int64_t TextMesh::node_count() const
{
  return m_data.nodeIds.size();
}

int64_t TextMesh::node_count_proc() const
{
  return m_data.num_nodes_on_proc(m_myProcessor);
}

int64_t TextMesh::block_count() const
{
  return m_data.partIds.size();
}

int64_t TextMesh::element_count() const
{
...
Topology TextMesh::get_topology_for_part(int64_t id) const
{
  ...
  ThrowRequireMsg(iter != m_partToTopology.end(), "Could not find a topology associated with part: " << partName);
...
void TextMesh::raw_node_map(std::vector<INT> &map) const
{
...
void TextMesh::node_map(Ioss::Int64Vector &map) const
{
  raw_node_map(map);
}

void TextMesh::node_map(Ioss::IntVector &map) const
{
  raw_node_map(map);
}
...
public:
  ...
  : m_myProcessor(myProc)
  , m_nodeMap(map)
  , m_processorMap(processors) {}
  ...
  void verify_map_size(const size_t minimumSize)
  {
    ...
    ThrowRequireMsg(m_processorMap.size() >= minimumSize, "Insufficient size in processor vector");
  }
...
template <typename INT>
void TextMesh::raw_element_map(int64_t id, std::vector<INT> &map) const
{
  auto iter = m_blockPartition.find(id);
  ThrowRequireMsg(iter != m_blockPartition.end(), "Could not find block with id: " << id << " in block partition");
...
void TextMesh::element_map(Ioss::Int64Vector &map) const
{
  raw_element_map(map);
}

void TextMesh::element_map(Ioss::IntVector &map) const
{
  raw_element_map(map);
}

template <typename INT>
void TextMesh::raw_element_map(std::vector<INT> &map) const
{
...
void TextMesh::connectivity(int64_t id, int64_t *connect) const
{
...
template <typename INT>
void TextMesh::raw_connectivity(int64_t id, INT *connect) const
{
  unsigned offset = 0;
  auto blockIter = m_blockPartition.find(id);
  ThrowRequireMsg(blockIter != m_blockPartition.end(), "Could not find block with id: " << id << " in block partition");

  for (const auto &elemId : blockIter->second.elemIds) {
    auto connIter = m_elementConnectivity.find(elemId);
    ThrowRequireMsg(
        connIter != m_elementConnectivity.end(), "Could not find element with id: " << id << " in connectivity map");
...
int64_t TextMesh::get_part_id(const std::string &name) const
{
  return m_data.partIds.get(name);
}
  ...
  for (const auto &elementData : m_data.elementDataVec) {
    ...
    if (iter == m_partToTopology.end()) {
      ...
      ThrowRequireMsg(iter->second == elementData.topology,
          "Element with id: " << elementData.identifier << " in part named: " << elementData.partName
