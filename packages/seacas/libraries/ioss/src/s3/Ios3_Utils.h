...
namespace Ios3 {
  ...
  IOS3_EXPORT key_t make_node_map_key(int rank,
                                      const std::string& name);
  IOS3_EXPORT key_t make_edge_map_search_key(int rank,
                                             const std::string& name="");
  IOS3_EXPORT key_t make_edge_map_key(int rank,
                                      const std::string& name);
  IOS3_EXPORT key_t make_face_map_search_key(int rank,
                                             const std::string& name="");
  IOS3_EXPORT key_t make_face_map_key(int rank,
                                      const std::string& name);
  IOS3_EXPORT key_t make_elem_map_search_key(int rank,
                                             const std::string& name="");
  IOS3_EXPORT key_t make_elem_map_key(int rank,
                                      const std::string& name);

  IOS3_EXPORT key_t make_states_search_key(int parallel_rank,
                                           const Ioss::Region &region);
  IOS3_EXPORT key_t make_states_key(int parallel_rank,
                                    const Ioss::Region &region);

  IOS3_EXPORT key_t sideblocks_search_key(int rank,
                                          const Ioss::Region &region,
                                          const Ioss::SideSet &sideset);
  IOS3_EXPORT key_t make_sideblock_key(int rank,
                                       const Ioss::Region &region,
                                       const Ioss::SideSet &sideset,
                                       const Ioss::SideBlock &sideblock);

  IOS3_EXPORT key_t
  structuredblock_search_key(int parallel_rank,
                             const Ioss::Region &region,
                             const Ioss::StructuredBlock &structuredblock);
  IOS3_EXPORT key_t
  make_structuredblock_key(int parallel_rank,
                           const Ioss::Region &region,
                           const Ioss::StructuredBlock &structuredblock);

  IOS3_EXPORT key_t make_key(int parallel_rank,
                             const Ioss::Region &region,
  ...
  IOS3_EXPORT key_t entity_search_key(int rank,
                                      const Ioss::Region &region,
  ...
  IOS3_EXPORT key_t property_search_key(int parallel_rank,
                                        const Ioss::Region &region,
                                        const Ioss::GroupingEntity &grouping_entity);
  IOS3_EXPORT key_t make_property_key(int rank,
                                      const Ioss::Region &region,
  ...
  IOS3_EXPORT key_t field_search_key(int parallel_rank,
                                     const Ioss::Region &region,
                                     const Ioss::GroupingEntity &grouping_entity);
  IOS3_EXPORT key_t field_search_key(int parallel_rank,
                                     int state,
  ...
  IOS3_EXPORT std::string get_entity_name(const std::string &k,
