...
std::shared_ptr<HelperContext> createContext(const HelperParameters &params)
{
  ...
  auto credProvider = Aws::MakeShared<AwsHelperCredentialsProviderChain>("CredProvider", params.profile.c_str());

  context->client = Aws::MakeShared<Aws::S3::S3Client>("S3Client",
                                                       credProvider,
                                                       config,
                                                       Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy::Never,
                                                       false /* disable virtual addressing - otherwise client requests timeout */);

  context->executor = Aws::MakeShared<Aws::Utils::Threading::PooledThreadExecutor>("executor", 25);
             ...
             const std::vector<T,A>        &value)
{
  ...
  std::shared_ptr<Aws::IOStream> input_stream = Aws::MakeShared<Aws::IOStream>("SomeTag", &streamBuffer);
             ...
             std::vector<T,A>              &value)
{
  ...
  if (outcome.IsSuccess()) {
    ...
    while ((bytes_left > 0) && (total_bytes_read < (int64_t)value.capacity())) {
      int64_t bytes_read = retrieved_obj_body.readsome((char*)&value[total_bytes_read], bytes_left);
