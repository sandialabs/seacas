...
namespace Ios3 {
  ...
  void DatabaseIO::get_step_times_nl()
  {
    ...
    rc = Ios3::helpers::listKeys(helper_context, bucket_name, search_key.second, keys);
    if (rc == 0 && keys.size() == 1) {
  ...
  {
    {
      ...
      std::vector<std::string> keys;
      int rc = Ios3::helpers::listKeys(helper_context, bucket_name, search_key.second, keys);
      if (rc) {
        std::ostringstream errmsg;
        errmsg << "S3 Operation Failed"
               << "\n";
  ...
  void DatabaseIO::get_edgeblocks()
  {
    ...
    for (auto entity_name : entity_names) {
      ...
      if (have_entity_count && have_original_edge_type) {
        ...
        rc = Ios3::helpers::listKeys(helper_context, bucket_name, property_search.second,
                                     property_keys);
        if (rc) {
          std::ostringstream errmsg;
          errmsg << "S3 Operation Failed"
        ...
        rc = Ios3::helpers::listKeys(helper_context, bucket_name, field_search.second, field_keys);
        if (rc) {
          std::ostringstream errmsg;
          errmsg << "S3 Operation Failed"
                 << "\n";
  ...
  void DatabaseIO::get_elemblocks()
  {
    ...
    for (auto entity_name : entity_names) {
      ...
      if (have_entity_count) {
        ...
        rc = Ios3::helpers::listKeys(helper_context, bucket_name, field_search_debug.second,
                                     field_keys_debug);
        if (rc) {
          std::ostringstream errmsg;
          errmsg << "S3 Operation Failed"
  ...
  void DatabaseIO::get_sidesets()
  {
    ...
    for (auto entity_name : entity_names) {
      ...
      rc = Ios3::helpers::listKeys(helper_context, bucket_name, sideblocks_key.second,
      ...
      for (size_t i = 0; i < sideblocks_search_keys.size(); i++) {
        ...
        rc = Ios3::helpers::getValue(helper_context, bucket_name, property_key.second,
                                     property_value);
        if (rc) {
          std::ostringstream errmsg;
          errmsg << "S3 Operation Failed"
        ...
        rc = Ios3::helpers::getValue(helper_context, bucket_name, parent_property_key.second,
        ...
        rc = Ios3::helpers::listKeys(helper_context, bucket_name, sideblock_property_search.second,
        ...
        rc = Ios3::helpers::listKeys(helper_context, bucket_name, sideblock_field_search.second,
  ...
  void DatabaseIO::get_commsets()
  {
    ...
    for (auto entity_name : entity_names) {
      ...
      rc = Ios3::helpers::listKeys(helper_context, bucket_name, field_search_transient.second,
                                         ...
                                         void *data, size_t data_size) const
  {
    ...
    if (count == 0) {
      if (field.get_name() == "connectivity_raw") {
        ...
        std::vector<std::string> keys;
        rc = Ios3::helpers::listKeys(helper_context, bucket_name, key.second, keys);
        if (rc) {
          std::ostringstream errmsg;
          errmsg << "S3 Operation Failed"
                 << "\n";
