...
std::vector<size_t> ParallelDatabaseIO::get_all_block_offset(const std::vector<int>& block_component_count) const
{
  return decomp->get_all_block_offset(block_component_count);
}

std::vector<int> ParallelDatabaseIO::get_all_block_connectivity_component_count() const
{
...
std::vector<size_t> ParallelDatabaseIO::get_all_block_connectivity(const std::string &field_name, void *data, size_t data_size) const
{
 ...
 std::vector<size_t> retval = decomp->get_all_block_connectivity(get_file_pointer(), data);

 if(field_name == "connectivity") {
   Ioss::Field::BasicType type = decomp->int_size() == sizeof(int) ? Ioss::Field::INTEGER : Ioss::Field::INT64;
...
std::vector<int> ParallelDatabaseIO::get_all_block_component_count(const std::string& field_name) const
{
 const Ioss::ElementBlockContainer& elem_blocks = get_region()->get_element_blocks();
 size_t num_blocks = elem_blocks.size();

 std::vector<int> block_component_count(num_blocks, 0);
 ...
 for(size_t i=0; i<num_blocks; i++) {
   ...
   if(entity->field_exists(field_name)) {
     ...
     block_component_count[iblk] = field.raw_storage()->component_count();
   }
 }

 return block_component_count;
}

size_t ParallelDatabaseIO::get_all_block_field_data_size(const std::string& field_name) const
{
 const Ioss::ElementBlockContainer& elem_blocks = get_region()->get_element_blocks();
 size_t num_blocks = elem_blocks.size();
 ...
 for(size_t i=0; i<num_blocks; i++) {
   ...
   if(entity->field_exists(field_name)) {
     Ioss::Field field = entity->get_field(field_name);
     total_data_size += entity->entity_count()*field.raw_storage()->component_count()*field.get_basic_size();
   }
 }
...
std::vector<size_t> ParallelDatabaseIO::get_all_block_transient_field_data(const Ioex::VariableNameMap &variables,
                                                                          const std::string &field_name, void *data) const
{
 std::vector<int> block_component_count = get_all_block_component_count(field_name);
 std::vector<size_t> ioss_offset = decomp->get_all_block_offset(block_component_count);
...
std::vector<size_t> ParallelDatabaseIO::get_all_block_field_data(const std::string &field_name, void *data, size_t data_size) const
{
 size_t expected_data_size = get_all_block_field_data_size(field_name);
