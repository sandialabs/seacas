...
namespace {
  ...
  inline std::vector<INT> get_entity_dist(int proc_count, INT entity_count)
  {
    std::vector<INT> dist(proc_count + 1);

    // Fill in entity_dist vector.  Range of entities on each processor...
  ...
  template <typename INT>
  std::vector<std::pair<INT, INT>> generate_node_communication(int proc_count, INT node_count) 
  {
    // We are in the pre-decomposition phase at this point which means that
    // Elements and nodes have been linearly distributed across ranks.
    // We can generate the `proc_entity` map here based on that linear
    // decomposition...
    auto node_dist = get_entity_dist(proc_count, node_count);

    std::vector<std::pair<INT, INT>> proc_entity;
    proc_entity.reserve(node_count);

    for (size_t p = 0; p < proc_count; p++) {
      auto begin = node_dist[p];
      auto end   = node_dist[p+1];
      for (size_t i = begin; i < end; i++) {
	proc_entity.push_back(std::make_pair(p, i));
      }
    }
    return proc_entity;
  }
  ...
  template <typename INT>
  void internal_generate_faces(int exoid, ex_block &eb, Ioss::FaceUnorderedSet &faces,
                               ...
                               bool local_ids, INT /*dummy*/)
  {
    ...
    auto topology =
        Ioss::Utils::fixup_type(eb.topology, eb.num_nodes_per_entry, 3);
    const Ioss::ElementTopology *topo = Ioss::ElementTopology::factory(topology);
    if (topo->parametric_dimension() != 3) {
      return;
    }

    std::vector<INT> connectivity(eb.num_entry * eb.num_nodes_per_entry);
    ex_get_conn(exoid, EX_ELEM_BLOCK, eb.id, connectivity.data(), nullptr, nullptr);

    std::vector<INT> elem_ids(eb.num_entry);
    ...
    if (local_ids) {
      ...
      ex_get_partial_id_map(exoid, EX_ELEM_MAP, eb_offset + 1, eb.num_entry,
                            elem_ids.data());
    }
                              ...
                              const std::vector<size_t> &hash_ids, INT /*dummy*/)
  {
    ...
    if (proc_count > 1) {
      ...
      std::vector<std::pair<INT, INT>> proc_entity = generate_node_communication(proc_count, (INT)hash_ids.size());
      Ioss::Map node_map("nodemap", "unknown", 0);
      internal_resolve_parallel_faces(node_map, faces, hash_ids, proc_entity, 
				      pu, INT(0));
    }
#endif
  }
...
namespace Ioex {
    ...
    void FaceGenerator::generate_faces(INT /*dummy*/, bool block_by_block, bool use_local_ids)
  {
    ...
    for (size_t i = 0; i < blk_cnt; i++) {
      ...
      ex_get_block_param(exodusFilePtr_, &ebs[i]);
    }
	...
	internal_generate_faces(exodusFilePtr_, eb, faces_[eb.id], ids, hashIds_, use_local_ids, (INT)0);
      }
      for (auto &eb : ebs) {
	auto  &my_faces = faces_[eb.id];
	resolve_parallel_faces(exodusFilePtr_, communicator_, my_faces, hashIds_, (INT)0);
      }
    }
    else {
      ...
      for (auto &eb : ebs) {
	internal_generate_faces(exodusFilePtr_, eb, my_faces, ids, hashIds_, use_local_ids, (INT)0);
      }
      resolve_parallel_faces(exodusFilePtr_, communicator_, my_faces, hashIds_, (INT)0);
    }
  }
