...
namespace doctest {
  ...
  // with length of up to 23 chars on the stack before going on the heap - the last byte of the
  // buffer is used for:
  ...
  namespace detail {
    ...
    template <typename T, unsigned long N> void filloss(std::ostream *stream, const T (&in)[N])
    {
...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)                              \
  template <typename R> DOCTEST_NOINLINE SFINAE_OP(Result, op) operator op(const R &&rhs)          \
  {                                                                                                \
    bool res =                                                                                     \
        op_macro(doctest::detail::forward<const L>(lhs), doctest::detail::forward<const R>(rhs));  \
...
#define DOCTEST_CMP_LE(l, r) le(l, r)
    ...
    struct Expression_lhs
    {
      ...
      // the comparisons, but the ExpressionDecomposer class uses the left shift operator to capture
    ...
    struct FatalConditionHandler
    {
      ...
      FatalConditionHandler()
      {
        ...
        std::set_terminate([]() DOCTEST_NOEXCEPT {
          ...
          if (isDebuggerActive() && !g_cs->no_breaks)
            DOCTEST_BREAK_INTO_DEBUGGER();
          std::exit(EXIT_FAILURE); // explicitly exit - otherwise the SIGABRT handler may be called
                                   // as well
        });
