...
TEST_CASE("two element ranges", "[two-element-ranges]")
{
...
TEST_CASE("two separated values pipe sep", "[two-sep-values-pipe]")
{
  std::string ret = Ioss::Utils::format_id_list({2, 11}, "--");
  REQUIRE(ret == std::string("2, 11"));
}
...
TEST_CASE("two ranges pipe separated", "[two-ranges-pipe]")
{
  std::string ret = Ioss::Utils::format_id_list({1, 2, 3, 4, 6, 7, 8}, "--", "|");
  REQUIRE(ret == std::string("1--4|6--8"));
}
...
TEST_CASE("large range with singles with words", "[large-range-w-singles-w-words]")
{
  std::vector<size_t> range(100000);
  std::iota(range.begin(), range.end(), 42);
  range[0] = 1;
