...
TEST_CASE("test single block", "[single_block]")
{
  ...
  int new_zones = pre_split(zones, avg_work, load_balance_tolerance, 0, proc_count);
}
...
TEST_CASE("test farmer plenum", "[farmer_plenum]")
{
  ...
  for(size_t proc_count=2; proc_count < 20; proc_count++) {
	...
	SECTION("assign_to_procs") {
	  ...
	  assign_zones_to_procs(zones, work_vector);
	  ...
	  for (size_t i=0; i < zones.size(); i++) {
	    if (zones[i]->is_active()) {
	      auto success = proc_adam_map.insert(std::make_pair(zones[i]->m_adam->m_zone,zones[i]->m_proc));
