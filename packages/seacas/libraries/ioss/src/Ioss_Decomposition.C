  ...
  template <typename INT> void Decomposition<INT>::guided_decompose()
  {
	...
	if (pos != std::string::npos) {
	  ...
	  if (scale == "AUTO" || scale == "auto") {
	    ...
	    if (m_processor == 0) {
	      fmt::print(Ioss::OUTPUT(), "IOSS: Element Processor Map automatic scaling factor = {}\n",
			 iscale);
	    }
	    if (iscale == 0) {
	      std::ostringstream errmsg;
		fmt::print(errmsg, "ERROR: Max value in element processor map is {} which is\n"
	...
	if (max_proc < m_processorCount - 1) {
	  if (m_processor == 0) {
	    fmt::print(Ioss::WARNING(), "Max value in element processor map is {} which is\n"
...
#if IOSS_DEBUG_OUTPUT
	fmt::print(Ioss::DEBUG(), "[{}] Export Count: {}\n", m_processor, fmt::join(exportElementCount, " "));
#endif
