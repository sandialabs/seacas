...
void IossApplication::printUsageMessage() {
    ...
    um += "-f <string> provide faodel configuration in <string> \n";
    um += "   format is ""config-file-name|resource-URI"" \n\n";
...
const std::string&
IossApplication::getFaodelParams() 
{
  return this->faodelParams;
}
...
void IossApplication::openInputIOSSDatabase() {
  ...
  hrclock::time_point then = hrclock::now();
    ...
    if( not faodelParams.empty() ) {
      /*
       * we expect this to be "config-file-name|resource-uri". split on the '\'.
       */
      std::string::size_type pos = faodelParams.find_first_of( '|' );
      std::string config_file_name = faodelParams.substr( 0, pos );
      std::string resource_uri = faodelParams.substr( pos + 1 );

      inputProperties.add( Ioss::Property( "faodel-config-append", config_file_name ) );
      inputProperties.add( Ioss::Property( "faodel-resource-uri", resource_uri ) );
    }
