...
namespace Iofaodel {
  ...
  void map_fields( const Ioss::Region & region, FieldFunction op)
  {
    ...
    for(auto name : description) {
      ...
      if(role == Ioss::Field::RoleType::TRANSIENT ||
          role == Ioss::Field::RoleType::REDUCTION)
      {
  ...
  field_entry_t::field_entry_t(
      const Ioss::Field & field, const size_t start)
      ...
      const Ioss::GroupingEntity & entity, const Ioss::Field & field)
  {
    ...
    meta_entry_t meta_entry{meta_entry_t::IossType::IossField, 0,
    ...
    std::memcpy(static_cast<char*>(ldo.GetDataPtr()), &field_entry,
    ...
    std::memcpy(storage_ptr, field.raw_storage()->name().data(),
        entry->storage.size);
    ...
    size_t data_size)
{
    ...
    std::memcpy(storage_ptr, f.raw_storage()->name().data(),
        entry->storage.size);
