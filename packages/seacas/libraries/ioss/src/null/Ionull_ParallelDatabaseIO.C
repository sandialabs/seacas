...
#include <exodus/Ionull_ParallelDatabaseIO.h>
...
#include <exodus/Ionull_DecompositionData.h>
...
#define NC_HAS_SZIP_WRITE 0
  ...
  void ParallelDatabaseIO::get_step_times__()
  {
...

    {
      ...
      Ionull::read_last_time_attribute(get_file_pointer(), &last_time);
    }
  ...
  const Ioss::Map &ParallelDatabaseIO::get_map(ex_entity_type type) const
  {
    ...
    default:
      std::ostringstream errmsg;
      fmt::print(errmsg,
                 "INTERNAL ERROR: Invalid map type. "
                 "Something is wrong in the Ionull::ParallelDatabaseIO::get_map() function. "
                 "Please report.\n");
      IOSS_ERROR(errmsg);
    }
  }
                                      ...
                                      const std::string &basename)
  {
    ...
    for (int iblk = 0; iblk < m_groupCount[entity_type]; iblk++) {
      ...
      if (entity_type == EX_ELEM_BLOCK) {
        if (nmap > 0) {
          nmap = Ionull::add_map_fields(get_file_pointer(),
  ...
  void ParallelDatabaseIO::get_sidesets()
  {
    ...
    if (m_groupCount[EX_SIDE_SET] > 0) {
      ...
      Ionull::SideSetMap ss_map;
      Ionull::SideSetSet ss_set;

      {
        ...
        for (const auto &ss : decomp->side_sets) {
          ...
          if (ss_name[0] != '\0') {
            ...
            Ionull::decode_surface_name(ss_map, ss_set, ss_name.data());
          }
        }
      }
      ...
      // IO system and are already split into homogeneous pieces...
      {
        for (auto &ss_name : ss_set) {
          ...
          int64_t id = Ionull::extract_id(ss_name);
          if (id > 0) {
              ...
              if (split_type == Ioss::SPLIT_BY_TOPOLOGIES) {
                ...
                if (block == nullptr || Ioss::Utils::block_is_omitted(block)) {
                  ...
                  fmt::print(
                      ...
                      "in the Ionull::ParallelDatabaseIO class. Please report.\n",
                      topo_or_block_name);
                  IOSS_ERROR(errmsg);
                }
                                                 ...
                                                 size_t data_size) const
  {
    ...
    int64_t id           = Ionull::get_id(sb, &ids_);
  ...
  int64_t ParallelDatabaseIO::read_transient_field(const Ionull::VariableNameMap &variables,
    ...
    for (int64_t iel = 0; iel < number_sides; iel++) {
      ...
      if (block == nullptr) {
        ...
        fmt::print(errmsg,
                   ...
                   " Something is wrong in the Ionull::ParallelDatabaseIO class. Please report.\n",
                   elem_id);
        IOSS_ERROR(errmsg);
      }
