...
namespace {
  ...
  int zoltan_num_dim(void *data, int *ierr)
  {
    ...
    Ioexnl::DecompositionDataBase *zdata = reinterpret_cast<Ioexnl::DecompositionDataBase *>(data);
...
namespace Ioexnl {
  ...
  template <typename INT> void DecompositionData<INT>::decompose_model(int filePtr)
  {
    ...
    m_decomposition.show_progress("\tFinished with Ioexnl::decompose_model");
    ...
    if (m_processor == set.root_) {
      ...
      if (field.get_name() == "ids" || field.get_name() == "ids_raw") {
        ...
        if (set.file_count() < max_size) {
          ...
          if (ierr < 0) {
            Ioexnl::exodus_error(filePtr, __LINE__, __func__, __FILE__);
          }
        }
        else {
