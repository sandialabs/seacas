...
#define NC_HAS_SZIP_WRITE 0
  ...
  void DatabaseIO::read_meta_data__()
  {
...

    {
      ...
      if (isParallel) {
        Ioexnl::check_processor_info(decoded_filename(), get_file_pointer(), util().parallel_size(),
                                     myProcessor);
      }
  ...
  void DatabaseIO::read_region()
  {
    ...
    Ioexnl::add_coordinate_frames(get_file_pointer(), this_region);
  ...
  void DatabaseIO::get_step_times__()
  {
    ...
    if (dbUsage == Ioss::WRITE_HISTORY) {
    ...
    else {
      {
        ...
        exists = Ioexnl::read_last_time_attribute(get_file_pointer(), &last_time);
      }
  ...
  const Ioss::Map &DatabaseIO::get_map(ex_entity_type type) const
  {
    ...
    default:
      ...
      fmt::print(errmsg, "INTERNAL ERROR: Invalid map type. "
                         "Something is wrong in the Ioexnl::DatabaseIO::get_map() function. "
                         "Please report.\n");
      IOSS_ERROR(errmsg);
    }
  }
                              ...
                              const std::string &basename)
  {
    ...
    for (iblk = 0; iblk < m_groupCount[entity_type]; iblk++) {
      ...
      if (ignore_database_names()) {
        ...
        Ioss::SerializeIO serializeIO__(this);
        block_name = Ioexnl::get_entity_name(get_file_pointer(), entity_type, id, basename,
      ...
      if (entity_type == EX_ELEM_BLOCK) {
        ...
        if (nmap > 0) {
          ...
          nmap = Ioexnl::add_map_fields(get_file_pointer(), elb, local_X_count[iblk],
                                        maximumNameLength);
        }
      }
    }
    ...
    if (m_groupCount[EX_SIDE_SET] > 0) {
      ...
      Ioss::Int64Vector side_set_ids(m_groupCount[EX_SIDE_SET]);
      {
        ...
        for (const auto &id : side_set_ids) {
          ...
          if (ss_name[0] != '\0') {
            ...
            Ioexnl::decode_surface_name(fs_map, fs_set, ss_name.data());
          }
        }
      }
      ...
      // IO system and are already split into homogeneous pieces...
      {
        for (const auto &fs_name : fs_set) {
          ...
          int64_t id = Ioexnl::extract_id(fs_name);
          if (id > 0) {
      ...
      for (int iss = 0; iss < m_groupCount[EX_SIDE_SET]; iss++) {
        ...
        Ioexnl::TopologyMap side_map; // Used to determine side consistency
        ...
        bool db_has_name = false;
        {
          ...
          if (ignore_database_names()) {
            ...
            side_set_name = Ioexnl::get_entity_name(get_file_pointer(), EX_SIDE_SET, id, "surface",
                                                    maximumNameLength, db_has_name);
          }
          ...
          if (!blockOmissions.empty() || !blockInclusions.empty()) {
            get_element_sides_lists(get_file_pointer(), id, int_byte_size_api(), number_sides,
                                    element, sides);
            Ioexnl::filter_element_list(get_region(), element, sides, true);
          ...
          if (split_type == Ioss::SPLIT_BY_TOPOLOGIES && sideTopology.size() == 1) {
            ...
            get_element_sides_lists(get_file_pointer(), id, int_byte_size_api(), number_sides,
                                    element, sides);
            Ioexnl::separate_surface_element_sides(element, sides, get_region(), topo_map, side_map,
              ...
              if (split_type == Ioss::SPLIT_BY_TOPOLOGIES) {
                ...
                if (block == nullptr || Ioss::Utils::block_is_omitted(block)) {
                  ...
                  fmt::print(errmsg,
                             ...
                             "in the Ioexnl::DatabaseIO class. Please report.\n",
                             topo_or_block_name);
                  IOSS_ERROR(errmsg);
                }
      ...
      std::vector<T *>  Xsets(count);
      {
        ...
        for (int ins = 0; ins < count; ins++) {
          ...
          if (ignore_database_names()) {
            ...
            Xset_name = Ioexnl::get_entity_name(get_file_pointer(), type, id, base + "list",
                                                maximumNameLength, db_has_name);
          }
          ...
          if ((!blockOmissions.empty() || !blockInclusions.empty()) && type == EX_NODE_SET) {
            ...
            compute_node_status();
            filtered = Ioexnl::filter_node_list(active_node_index, nodeConnectivityStatus);
  ...
  {
    return Ioexnl::BaseDatabaseIO::get_field_internal(reg, field, data, data_size);
  }
      ...
      if (role == Ioss::Field::MESH) {
        ...
        if (field.get_name() == "ids" || field.get_name() == "ids_raw") {
          if (field.get_type() == Ioss::Field::INTEGER) {
            ...
            if (ierr >= 0) {
              Ioexnl::filter_node_list(static_cast<int *>(data), dbvals,
            ...
            ierr = ex_get_set(get_file_pointer(), EX_NODE_SET, id, dbvals.data(), nullptr);
            if (ierr >= 0) {
              Ioexnl::filter_node_list(static_cast<int64_t *>(data), dbvals,
                                       activeNodeSetNodesIndex[ns->name()]);
            }
          }
          if (ierr < 0) {
          ...
          if (set_param[0].num_distribution_factor == 0) {
            ...
            Ioexnl::filter_node_list(static_cast<double *>(data), dbvals,
                                     activeNodeSetNodesIndex[ns->name()]);
            set_param[0].distribution_factor_list = nullptr;
          }
        }
  ...
  int64_t DatabaseIO::read_transient_field(const Ioexnl::VariableNameMap &variables,
    ...
    for (int64_t iel = 0; iel < number_sides; iel++) {
      ...
      if (block == nullptr) {
        ...
        fmt::print(errmsg,
                   ...
                   "Something is wrong in the Ioexnl::DatabaseIO class. Please report.\n",
                   elem_id);
        IOSS_ERROR(errmsg);
      }
    ...
    for (int complex_comp = 0; complex_comp < re_im; complex_comp++) {
      ...
      for (int i = 0; i < comp_count; i++) {
        ...
        if (ierr < 0) {
          ...
          Ioexnl::exodus_error(get_file_pointer(), __LINE__, __func__, __FILE__, errmsg.str());
        }
      }
    }
  }
                                                ...
                                                void *variables) const
  {
    ...
    if (comp_count == 1 && ioss_type == Ioss::Field::REAL && type != EX_SIDE_SET &&
      ...
      if (ierr < 0) {
        ...
        Ioexnl::exodus_error(get_file_pointer(), __LINE__, __func__, __FILE__, extra_info.str());
      }
      return;
    }
    int re_im = 1;
                                         ...
                                         void *data, size_t data_size) const
  {
    ...
    if (field.get_name() == "entity_processor") {
      ...
      if (type == "node") {
        ...
        if (commsetNodeCount > 0) {
          int ierr = ex_put_node_cmap(get_file_pointer(), Ioexnl::get_id(cs, &ids_),
        ...
        int ierr = ex_put_elem_cmap(get_file_pointer(), Ioexnl::get_id(cs, &ids_), entities.data(),
