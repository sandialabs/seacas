...
#define NC_HAS_SZIP_WRITE 0
  ...
  void ParallelDatabaseIO::get_step_times__()
  {
...

    {
      ...
      Ioexnl::read_last_time_attribute(get_file_pointer(), &last_time);
    }
  ...
  const Ioss::Map &ParallelDatabaseIO::get_map(ex_entity_type type) const
  {
    ...
    default:
      ...
      fmt::print(errmsg,
                 ...
                 "Something is wrong in the Ioexnl::ParallelDatabaseIO::get_map() function. "
                 "Please report.\n");
      IOSS_ERROR(errmsg);
    }
  }
                                      ...
                                      const std::string &basename)
  {
    ...
    for (int iblk = 0; iblk < m_groupCount[entity_type]; iblk++) {
      ...
      if (entity_type == EX_ELEM_BLOCK) {
        if (nmap > 0) {
          nmap = Ioexnl::add_map_fields(get_file_pointer(),
  ...
  void ParallelDatabaseIO::get_sidesets()
  {
    ...
    if (m_groupCount[EX_SIDE_SET] > 0) {
      ...
      Ioexnl::SideSetMap ss_map;
      Ioexnl::SideSetSet ss_set;

      {
        ...
        for (const auto &ss : decomp->side_sets) {
          ...
          if (ss_name[0] != '\0') {
            ...
            Ioexnl::decode_surface_name(ss_map, ss_set, ss_name.data());
          }
        }
      }
      ...
      // IO system and are already split into homogeneous pieces...
      {
        for (auto &ss_name : ss_set) {
          ...
          int64_t id = Ioexnl::extract_id(ss_name);
          if (id > 0) {
              ...
              if (split_type == Ioss::SPLIT_BY_TOPOLOGIES) {
                ...
                if (block == nullptr || Ioss::Utils::block_is_omitted(block)) {
                  ...
                  fmt::print(
                      ...
                      "in the Ioexnl::ParallelDatabaseIO class. Please report.\n",
                      topo_or_block_name);
                  IOSS_ERROR(errmsg);
                }
  ...
  int64_t ParallelDatabaseIO::read_transient_field(const Ioexnl::VariableNameMap &variables,
    ...
    for (int64_t iel = 0; iel < number_sides; iel++) {
      ...
      if (block == nullptr) {
        ...
        fmt::print(errmsg,
                   ...
                   " Something is wrong in the Ioexnl::ParallelDatabaseIO class. Please report.\n",
                   elem_id);
        IOSS_ERROR(errmsg);
      }
