...
namespace {
  ...
  template <typename T>
  Ioss::Field::RoleType verify_field_exists_on_any_entity_group(const std::string &field_name,
  ...
  template <typename T>
  size_t get_all_block_field_data_count(const std::string &field_name, const std::vector<T*> &entity_container)
  {
    size_t count = 0;
...
namespace Ioss {
                                                       ...
                                                       std::vector<T>    &field_data) const
  {
    ...
    Field::RoleType role = verify_field_exists_on_any_entity_group(field_name, *this, elem_blocks, "input");
    ...
    std::vector<size_t> offsets = internal_get_all_block_field_data(field_name, field_data.data(), data_size);
