...
namespace Ioss {
                                                                      ...
                                                                      Ioss::DatabaseUsage db_usage)
  {
    ...
    std::shared_ptr<DynamicTopologyNotifier> DynamicTopologyBroker::get_notifier(
    ...
    std::vector<std::shared_ptr<DynamicTopologyObserver>> DynamicTopologyBroker::get_observers(
        const std::string &model_name) const
    {
      std::vector<std::shared_ptr<DynamicTopologyObserver>> observers;
    ...
    void DynamicTopologyNotifier::register_observer(
        std::shared_ptr<DynamicTopologyObserver> observer)
    {
      observer->register_notifier(this);
      m_observers.push_back(observer);
    ...
    void DynamicTopologyNotifier::unregister_observer(
        std::shared_ptr<DynamicTopologyObserver> observer)
    {
      auto iter = std::find(m_observers.begin(), m_observers.end(), observer);
      if (iter != m_observers.end()) {
          ...
          m_dbChangeCount(region->get_topology_change_count())
    {
      if (nullptr == region) {
        std::ostringstream errmsg;
        fmt::print(errmsg,
                   "ERROR: null region passed in as argument to DynamicTopologyFileControl");
        IOSS_ERROR(errmsg);
      }
    ...
    bool DynamicTopologyFileControl::file_exists(
        const std::string &filename, const std::string &db_type, Ioss::DatabaseUsage db_usage)
    {
      return ::file_exists(util(), filename, db_type, db_usage);
    }
    ...
    std::string DynamicTopologyFileControl::get_linear_database_filename(
        const std::string &baseFileName, unsigned int step)
    {
      return ChangeSet::get_linear_database_filename(baseFileName, step);
    }

    std::string DynamicTopologyFileControl::construct_database_filename(
        int &step, Ioss::DatabaseUsage db_usage)
    {
      // Filename will be of the form -- ioDB-sxxxx where xxxx is step
      // number.  Assume maximum of 9999 steps (will do more, but won't have
      // good lineup of step numbers.
    ...
    bool DynamicTopologyFileControl::abort_if_exists(
        const std::string &filename, const std::string &db_type, Ioss::DatabaseUsage db_usage)
    {
    ...
    Ioss::DatabaseIO *DynamicTopologyFileControl::clone_output_database(int steps)
    {
      ...
      Ioss::DatabaseIO *db = Ioss::IOFactory::create(
          m_dbType, filename, db_usage, current_db->util().communicator(), m_properties);
    ...
    DynamicTopologyStateLocator::DynamicTopologyStateLocator(
        Ioss::DatabaseIO * db, const std::string &dbName, const std::string &dbType,
        unsigned fileCyclicCount, bool loadAllFiles)
    ...
    DynamicTopologyStateLocator::DynamicTopologyStateLocator(
        Ioss::DatabaseIO * db, unsigned fileCyclicCount, bool loadAllFiles)
        : m_database(db),
    ...
    std::tuple<std::string, int, double> DynamicTopologyStateLocator::locate_db_state(
    ...
    void DynamicTopologyStateLocator::locate_db_state_impl(double targetTime, DatabaseState &loc)
        const
    {
      auto changeSet =
    ...
    void DynamicTopologyStateLocator::get_db_time_impl(
        double init_time, StateLocatorCompare comparator, DatabaseState &loc) const
    {
      auto changeSet =
          Ioss::ChangeSetFactory::create(m_database, m_ioDB, m_dbType, m_fileCyclicCount);
      changeSet->populate_change_sets(m_loadAllFiles);
