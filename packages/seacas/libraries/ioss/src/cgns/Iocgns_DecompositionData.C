...
namespace Iocgns {
  ...
  template <typename INT> void DecompositionData<INT>::decompose_model(int cgnsFilePtr)
  {
    ...
    std::vector<INT> element_dist = get_entity_dist<INT>(
        processorCount, myProcessor, globalElementCount, &elementOffset, &elementCount);

    // Does not yet take into account shared nodes at zone boundaries.
    std::vector<INT> node_dist =
        get_entity_dist<INT>(processorCount, myProcessor, globalNodeCount, &nodeOffset, &nodeCount);
  ...
  template <typename INT>
  void DecompositionData<INT>::generate_zone_shared_nodes(int cgnsFilePtr, INT min_node,
                                                          INT max_node)
  {
  ...
  template <typename INT>
  void DecompositionData<INT>::generate_adjacency_list(int cgnsFilePtr, std::vector<INT> &pointer,
    ...
    for (int block = 0; block < block_count; block++) {
      ...
      if (b_start < p_end && p_start < b_end) {
        ...
        std::cerr << "Processor " << myProcessor << " has " << overlap
                  << " elements on element block " << block << "\t(" << blk_start << " to "
                  << blk_end << ")\n";
#endif
        el_blocks[block].fileSectionOffset = blk_start;
        cg_elements_partial_read(cgnsFilePtr, base, zone, section, blk_start, blk_end,
  ...
  template <typename INT> void DecompositionData<INT>::get_element_block_communication()
  {
    for (auto &block : el_blocks) {
...
#if DEBUG_OUTPUT
    std::cerr << "Processor " << myProcessor << " has " << nodeCommMap.size() << " shared nodes\n";
#endif
  }

  template <typename INT>
  void DecompositionData<INT>::get_file_node_coordinates(int cgnsFilePtr, int direction,
                                                         double *data) const
  {
    ...
    for (int zone = 1; zone <= num_zones; zone++) {
      ...
      if (end > node_offset && beg <= node_offset + node_count) {
        ...
        if (finish > start) {
          ...
          int ierr = cg_coord_read(cgnsFilePtr, base, zone, coord_name[direction].c_str(),
  ...
  template <typename INT>
  void DecompositionData<INT>::get_node_coordinates(int cgnsFilePtr, double *ioss_data,
  ...
  template void DecompositionData<int>::get_block_connectivity(int cgnsFilePtr, int *data,
                                                               int blk_seq) const;
  template void DecompositionData<int64_t>::get_block_connectivity(int cgnsFilePtr, int64_t *data,
                                                                   int blk_seq) const;
  ...
  void DecompositionData<INT>::get_block_connectivity(int cgnsFilePtr, INT *data, int blk_seq) const
  {
    ...
    int                   base = 1;
    cg_elements_partial_read(cgnsFilePtr, base, blk.zone(), blk.section(), blk.fileSectionOffset,
  ...
  template void DecompositionData<int64_t>::communicate_block_data(cgsize_t *file_data,
                                                                   int64_t * ioss_data,
                                                                   size_t    blk_seq,
                                                                   size_t    comp_count) const;
  template void DecompositionData<int>::communicate_block_data(cgsize_t *file_data, int *ioss_data,
                                                               size_t blk_seq,
                                                               size_t comp_count) const;
  template <typename INT>
  template <typename T>
  void DecompositionData<INT>::communicate_block_data(cgsize_t *file_data, T *ioss_data,
                                                      size_t blk_seq, size_t comp_count) const
  {
