    ...
    if (m_cgnsFilePtr < 0) {
      ...
      if (!is_input()) {
        if (m_cgnsFilePtr == -2) {
          ...
          if (open_create_behavior() == Ioss::DB_APPEND || open_create_behavior() == Ioss::DB_MODIFY) {
...
//
  ...
  bool ParallelDatabaseIO::end__(Ioss::State state)
  {
    ...
    case Ioss::STATE_DEFINE_MODEL:
      if (!is_input() && open_create_behavior() != Ioss::DB_APPEND && open_create_behavior() != Ioss::DB_MODIFY) {
        write_meta_data();
      }
