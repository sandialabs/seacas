...
public:
  ...
  void define_model(Ioss::Region& region) override
  {
    ::define_model(inputRegion, region);
  }

  void write_model(Ioss::Region& region) override
  {
    ::write_model(inputRegion, region);
  }
...
void run_simple_topology_change(const Ioss::Region& i_region,
...
void run_multi_file_simple_topology_change(const std::string &elemFieldName, const std::string &outFile)
{
...
void run_single_file_simple_topology_change(const std::string &elemFieldName, const std::string &outFile)
{
  Ioss::Init::Initializer io;
  Ioss::ParallelUtils util(Ioss::ParallelUtils::comm_world());
...
void read_and_test_single_file_simple_topology_change(const std::string& elemFieldName, const std::string& outFile)
{
  Ioss::PropertyManager propertyManager;

  Ioss::DatabaseIO *i_database = Ioss::IOFactory::create("exodus", outFile, Ioss::READ_RESTART,
                                                         Ioss::ParallelUtils::comm_world(),
                                                         propertyManager);
  ...
  test_single_file_simple_topology_change_data(i_region, elemFieldName, gold_min_step, gold_min_time);
  ...
  test_single_file_simple_topology_change_data(i_region, elemFieldName, gold_max_step, gold_max_time);
}
