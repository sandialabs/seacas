...
int main(int argc, char **argv)
{
  ...
  int  num_side_sets;
  EXCHECK(ex_get_init(exoid, title, &num_dim, &num_nodes, &num_elem, &num_elem_blk, &num_node_sets,
                      &num_side_sets));
  ...
  EXCHECK(ex_get_coord(exoid, x, y, z));
  ...
  EXCHECK(ex_get_coord_names(exoid, coord_names));
  ...
  {
    int num_attrs = 0;
    EXCHECK(ex_get_attr_param(exoid, EX_NODAL, 0, &num_attrs));
    ...
    if (num_attrs > 0) {
      ...
      EXCHECK(ex_get_attr_names(exoid, EX_NODAL, 0, attrib_names));

      float *attrib = (float *)calloc(num_nodes, sizeof(float));
      for (int j = 0; j < num_attrs; j++) {
        printf("nodal attribute %d = '%s'\n", j, attrib_names[j]);
        EXCHECK(ex_get_one_attr(exoid, EX_NODAL, 0, j + 1, attrib));
        for (int i = 0; i < num_nodes; i++) {
          printf("%5.1f\n", attrib[i]);
        }
        free(attrib_names[j]);
  ...
  EXCHECK(ex_get_id_map(exoid, EX_ELEM_MAP, elem_map));
  ...
  if (num_elem_blk > 0) {
    ...
    for (int i = 0; i < num_elem_blk; i++) {
      ...
      EXCHECK(ex_get_block_id_map(exoid, EX_ELEM_BLOCK, ids[i], block_map));
    ...
    EXCHECK(ex_get_prop_names(exoid, EX_ELEM_BLOCK, prop_names));

    for (int i = 1; i < num_props; i++) /* Prop 1 is id; skip that here */
    {
      for (int j = 0; j < num_elem_blk; j++) {
        int prop_value;
        EXCHECK(ex_get_prop(exoid, EX_ELEM_BLOCK, ids[j], prop_names[i], &prop_value));
    ...
    for (int i = 0; i < num_elem_blk; i++) {
      if (num_elem_in_block[i] > 0) {
        ...
        EXCHECK(ex_get_conn(exoid, EX_ELEM_BLOCK, ids[i], connect, NULL, NULL));
    ...
    for (int i = 0; i < num_elem_blk; i++) {
      if (num_elem_in_block[i] > 0 && num_attr[i] > 0) {
        ...
        EXCHECK(ex_get_attr(exoid, EX_ELEM_BLOCK, ids[i], attrib));

        EXCHECK(ex_get_attr_names(exoid, EX_ELEM_BLOCK, ids[i], attrib_names));
  ...
  if (num_node_sets > 0) {
    ...
    EXCHECK(ex_get_ids(exoid, EX_NODE_SET, ids));
    ...
    EXCHECK(ex_get_names(exoid, EX_NODE_SET, nset_names));
    ...
    for (int i = 0; i < num_node_sets; i++) {
      ...
      int num_df_in_set;
      EXCHECK(ex_get_set_param(exoid, EX_NODE_SET, ids[i], &num_nodes_in_set, &num_df_in_set));
      ...
      EXCHECK(ex_get_set(exoid, EX_NODE_SET, ids[i], node_list, NULL));

      if (num_df_in_set > 0) {
        EXCHECK(ex_get_set_dist_fact(exoid, EX_NODE_SET, ids[i], dist_fact));
      }
      ...
      {
        int num_attrs = 0;
        EXCHECK(ex_get_attr_param(exoid, EX_NODE_SET, ids[i], &num_attrs));
        ...
        if (num_attrs > 0) {
          ...
          EXCHECK(ex_get_attr_names(exoid, EX_NODE_SET, ids[i], attrib_names));
          float *attrib = (float *)calloc(num_nodes_in_set, sizeof(float));
          for (int j = 0; j < num_attrs; j++) {
            printf("nodeset attribute %d = '%s'\n", j, attrib_names[j]);
            EXCHECK(ex_get_one_attr(exoid, EX_NODE_SET, ids[i], j + 1, attrib));
    ...
    EXCHECK(ex_get_prop_names(exoid, EX_NODE_SET, prop_names));

    for (int i = 0; i < num_props; i++) {
      EXCHECK(ex_get_prop_array(exoid, EX_NODE_SET, prop_names[i], prop_values));
  ...
  if (num_node_sets > 0) {
...

    {
      ...
      EXCHECK(ex_get_concat_sets(exoid, EX_NODE_SET, &set_specs));
    }
  ...
  if (num_side_sets > 0) {
    ...
    EXCHECK(ex_get_ids(exoid, EX_SIDE_SET, ids));
    ...
    EXCHECK(ex_get_names(exoid, EX_SIDE_SET, sset_names));
    ...
    for (int i = 0; i < num_side_sets; i++) {
      ...
      int num_df_in_set;
      EXCHECK(ex_get_set_param(exoid, EX_SIDE_SET, ids[i], &num_sides_in_set, &num_df_in_set));
      ...
      EXCHECK(ex_get_set(exoid, EX_SIDE_SET, ids[i], elem_list, side_list));
      EXCHECK(ex_get_side_set_node_list(exoid, ids[i], node_ctr_list, node_list));

      if (num_df_in_set > 0) {
        EXCHECK(ex_get_set_dist_fact(exoid, EX_SIDE_SET, ids[i], dist_fact));
      }
    ...
    EXCHECK(ex_get_prop_names(exoid, EX_SIDE_SET, prop_names));

    for (int i = 0; i < num_props; i++) {
      for (int j = 0; j < num_side_sets; j++) {
        int prop_value;
        EXCHECK(ex_get_prop(exoid, EX_SIDE_SET, ids[j], prop_names[i], &prop_value));
  ...
  if (num_side_sets > 0) {
    ...
    EXCHECK(ex_get_concat_sets(exoid, EX_SIDE_SET, &set_specs));
  ...
  EXCHECK(ex_get_qa(exoid, qa_record));
  printf("QA records = \n");
  for (int i = 0; i < num_qa_rec; i++) {
    for (int j = 0; j < 4; j++) {
      printf(" '%s'\n", qa_record[i][j]);
  ...
  EXCHECK(ex_get_info(exoid, info));
  ...
  int num_glo_vars;
  EXCHECK(ex_get_variable_param(exoid, EX_GLOBAL, &num_glo_vars));
  ...
  if (num_glo_vars > 0) {
    ...
    EXCHECK(ex_get_variable_names(exoid, EX_GLOBAL, num_glo_vars, var_names));
  ...
  int num_nod_vars = 0;
  if (num_nodes > 0) {
    EXCHECK(ex_get_variable_param(exoid, EX_NODAL, &num_nod_vars));
    ...
    EXCHECK(ex_get_variable_names(exoid, EX_NODAL, num_nod_vars, var_names));
  ...
  int num_ele_vars = 0;
  if (num_elem > 0) {
    EXCHECK(ex_get_variable_param(exoid, EX_ELEM_BLOCK, &num_ele_vars));
    char *var_names[3];
    for (int i = 0; i < num_ele_vars; i++) {
      var_names[i] = (char *)calloc((MAX_STR_LENGTH + 1), sizeof(char));
    }

    EXCHECK(ex_get_variable_names(exoid, EX_ELEM_BLOCK, num_ele_vars, var_names));
    ...
    if (num_ele_vars > 0) {
      ...
      EXCHECK(ex_get_truth_table(exoid, EX_ELEM_BLOCK, num_elem_blk, num_ele_vars, truth_tab));
  ...
  int num_nset_vars = 0;
  if (num_node_sets > 0) {
    EXCHECK(ex_get_variable_param(exoid, EX_NODE_SET, &num_nset_vars));
    if (num_nset_vars > 0) {
      char *var_names[3];
      ...
      EXCHECK(ex_get_variable_names(exoid, EX_NODE_SET, num_nset_vars, var_names));
      ...
      if (num_nset_vars > 0) {
        ...
        EXCHECK(ex_get_truth_table(exoid, EX_NODE_SET, num_node_sets, num_nset_vars, truth_tab));
  ...
  int num_sset_vars = 0;
  if (num_side_sets > 0) {
    EXCHECK(ex_get_variable_param(exoid, EX_SIDE_SET, &num_sset_vars));
    if (num_sset_vars > 0) {
      char *var_names[3];
      ...
      EXCHECK(ex_get_variable_names(exoid, EX_SIDE_SET, num_sset_vars, var_names));
      ...
      if (num_sset_vars > 0) {
        ...
        EXCHECK(ex_get_truth_table(exoid, EX_SIDE_SET, num_side_sets, num_sset_vars, truth_tab));
  ...
  int   time_step = 3;
  float time_value;
  EXCHECK(ex_get_time(exoid, time_step, &time_value));
  ...
  EXCHECK(ex_get_all_times(exoid, time_values));
  ...
  if (num_glo_vars > 0) {
    ...
    EXCHECK(ex_get_var(exoid, time_step, EX_GLOBAL, 1, 1, num_glo_vars, var_values));
    ...
    EXCHECK(ex_get_var_time(exoid, EX_GLOBAL, var_index, 1, beg_time, end_time, var_values));
  ...
  if (num_nodes > 0) {
    ...
    EXCHECK(ex_get_var(exoid, time_step, EX_NODAL, var_index, 1, num_nodes, var_values));
    ...
    int node_num = 1;
    EXCHECK(ex_get_var_time(exoid, EX_NODAL, var_index, node_num, beg_time, end_time, var_values));
  ...
  if (num_elem_blk > 0) {
    ...
    for (int i = 0; i < num_elem_blk; i++) {
      if (num_elem_in_block[i] > 0) {
        ...
        EXCHECK(ex_get_var(exoid, time_step, EX_ELEM_BLOCK, var_index, ids[i], num_elem_in_block[i],
                           var_values));
  ...
  if (num_sset_vars > 0) {
    ...
    for (int i = 0; i < num_side_sets; i++) {
      ...
      EXCHECK(ex_get_var(exoid, time_step, EX_SIDE_SET, var_index, ids[i], num_elem_per_set[i],
                         var_values));
  ...
  if (num_nset_vars > 0) {
    ...
    for (int i = 0; i < num_node_sets; i++) {
      ...
      EXCHECK(ex_get_var(exoid, time_step, EX_NODE_SET, var_index, ids[i], num_nodes_per_set[i],
                         var_values));
