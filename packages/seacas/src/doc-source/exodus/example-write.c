...
int main(int argc, char **argv)
{
    ...
    error = ex_put_init(exoid, "This is a test", num_dim,
    ...
    error = ex_put_elem_block(exoid, ebids[0], "quad",
			      num_elem_in_block[0],
			      num_nodes_per_elem[0], 1);
    error = ex_put_elem_block(exoid, ebids[1], "quad",
			      num_elem_in_block[1],
			      num_nodes_per_elem[1], 1);
    error = ex_put_elem_block(exoid, ebids[2], "hex",
			      num_elem_in_block[2],
			      num_nodes_per_elem[2], 1);
    error = ex_put_elem_block(exoid, ebids[3], "tetra",
			      num_elem_in_block[3],
			      num_nodes_per_elem[3], 1);

    /* Use alternative function to do same thing... */
    error = ex_put_block(exoid, EX_ELEM_BLOCK, ebids[4], "wedge",
			 num_elem_in_block[4], num_nodes_per_elem[4],
			 0, 0, 1);
    error = ex_put_block(exoid, EX_ELEM_BLOCK, ebids[5], "tetra",
			 num_elem_in_block[5], num_nodes_per_elem[5],
			 0, 0, 1);
    error = ex_put_block(exoid, EX_ELEM_BLOCK, ebids[6], "tri",
			 num_elem_in_block[6], num_nodes_per_elem[6],
			 0, 0, 1);
    ...
    connect[0] = 5;    connect[1] = 6;
    connect[2] = 7;    connect[3] = 8;
    error = ex_put_conn(exoid, EX_ELEM_BLOCK, ebids[1], connect, 0, 0);

    connect[0] = 9;     connect[1] = 10;
    connect[2] = 11;    connect[3] = 12;
    connect[4] = 13;    connect[5] = 14;
    connect[6] = 15;    connect[7] = 16;
    error = ex_put_conn(exoid, EX_ELEM_BLOCK, ebids[2], connect, 0, 0);

    connect[0] = 17;    connect[1] = 18;
    connect[2] = 19;    connect[3] = 20;
    error = ex_put_conn(exoid, EX_ELEM_BLOCK, ebids[3], connect, 0, 0);

    connect[0] = 21;    connect[1] = 22;
    connect[2] = 23;    connect[3] = 24;
    connect[4] = 25;    connect[5] = 26;
    error = ex_put_conn(exoid, EX_ELEM_BLOCK, ebids[4], connect, 0, 0);

    connect[0] = 17;    connect[1] = 18;
    connect[2] = 19;    connect[3] = 20;
    connect[4] = 27;    connect[5] = 28;
    connect[6] = 30;    connect[7] = 29;
    error = ex_put_conn(exoid, EX_ELEM_BLOCK, ebids[5], connect, 0, 0);

    /* Use "old" API function just to show syntax */
    connect[0] = 31;    connect[1] = 32;
    connect[2] = 33;
    ...
    for (i = 0; i < 7; i++) {
      error = ex_put_attr_names(exoid, EX_ELEM_BLOCK,
				ebids[i], attrib_names);
    }
    ...
    error = ex_put_variable_names(exoid, EX_ELEM_BLOCK,
    ...
    error = ex_put_variable_names(exoid, EX_NODE_SET,
    ...
    error = ex_put_variable_names(exoid, EX_SIDE_SET,
    ...
    error = ex_put_truth_table(exoid, EX_ELEM_BLOCK,
			       num_elem_blk, num_ele_vars, truth_tab);
    free(truth_tab);
	...
	error = ex_put_var(exoid, whole_time_step, EX_GLOBAL,
			   1, 0, num_glo_vars, glob_var_vals);

	/* write nodal variables */
	for (k = 1; k <= num_nod_vars; k++) {
	    for (j = 0; j < num_nodes; j++) {
		nodal_var_vals[j] =
		    (float) k + ((float) (j + 1) * time_value);
	    }
	    error = ex_put_var(exoid, whole_time_step, EX_NODAL,
			       k, 1, num_nodes, nodal_var_vals);
	}
