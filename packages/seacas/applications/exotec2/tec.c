...
void tec(int exoid, const char *filename) {
  ...
  for (i=0; i<nblk; i++) {
    ex_get_elem_block(exoid,elem_id[i],elem_type[i],&elem_per_blk[i],
		      &node_per_elem[i],&attr_per_blk[i]);
  ...
  */
  ...
  if (izone == 0) {
	...
	i=0;
	teczone(1,nnode,j+1,elem_type[i],node_per_elem[i],
		nelem,ic,ndim,x,nvar,q,tecfile); 
      }
      printf("\n");
    }
	...
	ex_get_nodal_var(exoid,itime,i+1,nnode,q[i]);
      ...
      for (i=0; i<nblk; i++)
	teczone(nblk,nnode,elem_id[i],elem_type[i],
		node_per_elem[i],elem_per_blk[i],icon[i],
 ...
 */
void teczone(int nblk, int nnode, 
             int elem_id, char *elem_type, 
             int node_per_elem, int elem_per_blk, 
             int *icon, int ndim, double **x,
             int nvar, double **q, FILE *tecfile) {
  ...
  printf("    Writing Zone %4d, %8s, %6d elements, %6d nodes\n",
         elem_id,elem_type,elem_per_blk,inode);
