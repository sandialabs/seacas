...
IossMesh::IossMesh()
  : m_communicator(Ioss::ParallelUtils::comm_world())
{

}

IossMesh::IossMesh(Ioss_MPI_Comm comm)
  : m_communicator(comm)
{

}

int IossMesh::get_parallel_size()
{
  return Ioss::ParallelUtils(get_comm()).parallel_size();
}

int IossMesh::get_parallel_rank()
{
...
void IossMesh::fill_mesh(const std::string &meshDesc)
{
  fill_mesh(meshDesc, "input_region");
}
...
void IossMesh::create_database(const std::string &fileName, const std::string &meshType, Ioss::DatabaseUsage dbUsage)
{
  if (m_database == nullptr) {
    std::string meshFileName = fileName;
    filename_substitution(meshFileName);
    m_database = Ioss::IOFactory::create(meshType, meshFileName, dbUsage, get_comm(),
                                         m_propertyManager);
    EXPECT_TRUE(m_database != nullptr);
