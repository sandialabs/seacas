...
C    See packages/seacas/LICENSE for details
      ...
      SUBROUTINE DTVOL3D( COORD, DISP, LSTSN, NSEG, DELVOL, NDIM, NUMNP)
...
C     This routine is for tets which have triangular faces.      
...
C ... We treat the wedge (triangular face displaced) as a degenerate hex
C     so can use the same formula.  Collapse nodes 3,4 and 7,8
      DO 100 KSEG = 1 , NSEG
         K = LSTSN(3*KSEG )
         J = LSTSN(3*KSEG - 1)
         I = LSTSN(3*KSEG - 2)
