...
#define PREALLOCATION_SIZE 10
  ...
  if (i>=_MPI_COMM_ARRAY_SIZE) {
    _MPI_COVERAGE();
    _MPI_OP_LIST = (_MPI_OP_TYPE*) _MPI_safeRealloc(_MPI_OP_LIST, (_MPI_OP_ARRAY_SIZE+PREALLOCATION_SIZE)*sizeof(_MPI_OP_TYPE), "Error in _MPI_Op_Insert for reallocation"); 
