...
C=======================================================================
C $Id: lnksto.f,v 1.1 2008/10/31 05:04:00 gdsjaar Exp $
C $Log: lnksto.f,v $
...
c         IF ((TYPVAR(IVAR) .EQ. 'T')
c     &      .OR. (TYPVAR(IVAR) .EQ. 'H')
c     &      .OR. (TYPVAR(IVAR) .EQ. 'G')) THEN
            DO 100 ITM = 1, 3
...
c      IXHV = 0
      ...
      DO 140 IVAR = 1, NUMINP
         IF (TYPVAR(IVAR) .EQ. 'G') THEN
            IXGV = 1
         END IF
c         IF (TYPVAR(IVAR) .EQ. 'H') THEN
c            IXHV = 1
c         ELSE IF (TYPVAR(IVAR) .EQ. 'G') THEN
c            IXGV = 1
c         END IF
  140 CONTINUE
...
c            ELSE IF (TYPVAR(IVAR) .EQ. 'H') THEN
C            --Input history start at assigned slot
c               IDVAR(IVAR) = IXHV
...
C            --Share slot with input history variable of the same name (if any)
C            --or reserve new slot

c               IF (IXHV .GT. 0) THEN
c                  IINP = LOCSTR (NAMVAR(IVAR), NVARHI, NAMEHV)
c               ELSE
c                  IINP = 0
c               END IF
c               IF (IINP .GT. 0) THEN
c                  IDVAR(IVAR) = IINP + IXHV - 1
c               ELSE
c                  LTMENT = LTMENT + 1
c                  IDVAR(IVAR) = LTMENT
c               END IF
