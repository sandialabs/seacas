...
C    
C=======================================================================
      SUBROUTINE RWEVAL (NDBIN, NDBOUT, A, C, NPTIMS, NUMSTO,
     &                   LTMENT, MAXSTK, NWRIT, IOERR, MERR)
C=======================================================================
C $Id: rweval.f,v 1.1 2008/10/31 05:04:00 gdsjaar Exp $
C $Log: rweval.f,v $
...
C
C Revision 1.20  2008/03/14 13:45:29  gdsjaar
C Added copyright information to top of all files.
C
C ALGEBRA2 is now open-sourced under BSD license
C
C Revision 1.19  2000/12/05 22:28:56  gdsjaar
C Fix problem with incorrect array size for node set distribution factor
C indices.  Also wrong size in mdget call.
C
C Revision 1.18  2000/09/21 20:33:11  gdsjaar
C Changes to work with files that have been decomposed and do not have
C any nodesets or sidesets on this file even though the combined file
C has sidesets and/or nodesets.  Added checks that number of
C nodes/elements in nodesets/sidesets was positive before calling some
C routines.
C
C This problem was causing failures on pronto3d history files in a
C parallel run.
C
C Revision 1.17  1997/09/08 15:54:32  gdsjaar
C Added missing variable to argument list.
C Added initialization of the variable
C
C Revision 1.16  1996/05/07 22:00:53  caforsy
C Fixed bug in eval that set the size of an array greater than 1 when
C only 1 value was being processed by dooper.  Caused core dump on cray.
C Changed rweval to make sure that all the mdfind were done after
C a mdcomp.
C
C Revision 1.15  1996/02/14  18:13:27  caforsy
C Fixed error after mdcomp(). Need to find time step array indices.(IPTIMS)
C
C Revision 1.14  1996/02/12 21:28:47  caforsy
C Removed redundant checks for ExodusII API calls
C
C Revision 1.13  1996/01/15 17:37:21  caforsy
C Enable algebra2 to accept header data from exodusII file and process.
C Fixed print statement.
C
C Revision 1.12  1995/12/10 15:46:53  caforsy
C Integrated call to rpterr and mdcomp/mccomp calls
C
C Revision 1.11  1995/10/26 20:37:47  caforsy
...
C
C Revision 1.10  1995/10/25  16:33:56  caforsy
C Added error checking during results variable read/write
C
C Revision 1.9  1995/10/03 21:36:37  mksmith
...
C     Find element block indices of arrays already reserved
      ...
      CALL MCFIND ('NAMELB', KNMLB, NELBLK * MXSTLN)
      ...
      END IF
      CALL MCFIND ('NAMECO', KNACOR, MXSTLN*NDIM)
...
C     if (num_blks_out < num_blks_in or is_a_zoom_mesh) then
c      IF ((NUMEQL (.TRUE., NELBLK, VISELB) .LT. NELBLK)
...
C              Reserve scratch arrays for ZMESS
               ...
               CALL MDRSRV ('IXNSS', KXNSS, LESSNL)
               CALL MDSTAT (NERR, MEM)
               IF (NERR .GT. 0) THEN
                  CALL MEMERR
                  MERR = 1
...
C              CONVERT SIDES TO NODES
               CALL DBIGN (NDBIN, NUMESS, A(KIDSS), A(KNNSS), 
     &                  A(KIXNSS), A(KLTNSS), A(KLTNNN))

               CALL ZMESS (NUMEL, NUMELO, A(KXELEM),
     &            NUMNP, NUMNPO, A(KXNODE), NESSO, LESSEO, LESSNO,
     &            A(KIDSS), A(KNESS), A(KNNSS), A(KIXESS), A(KIXNSS),
     &            A(KLTESS), A(KLTSSS), A(KLTNSS), A(KFACSS), A(KNDSS),
     &            A(KNEWIX), A(KNEWSD), A(KXESS), A(KXNSS))
...
C ****************************************************************
c      IF ((NUMNP .EQ. NUMNPO) .AND. (.NOT. ISZOOM)) THEN
...
C     Write the node set information
      if ((numnps .gt. 0) .AND. (nnpso .gt. 0)
     &  .and. (lnpsnl .gt. 0)) then
...
C ****************************************************************
c      IF ((NUMEL .EQ. NUMELO) .AND. (.NOT. ISZOOM)) THEN
      IF (NUMEL .EQ. NUMELO) THEN
        IF (numess .gt. 0 .and. lessel .gt. 0) THEN
C          Reserve Dynamic arrays needed to process information
           CALL MDGET  (5*NUMESS + 2*LESSEL + LESSNL)
C          IDESS - array containing side set IDS
           CALL MDRSRV ('IDESS' , KIDSS , NUMESS)
...
C     Write the side set information
      IF ((numess .gt. 0) .AND. (nesso .gt. 0) .and. (lessel .gt. 0))
     &  THEN
...
C     Cumulative element counts for each output block
      ...
      CALL MDFIND ('NUMELB', KNELB, NELBLK)
      CALL MCFIND ('NAMECO', KNACOR, MXSTLN*NDIM)
      CALL MCFIND ('NAMES', KNAMES, MXSTLN*(NVARGL+NVARNP+NVAREL))
      CALL MCFIND ('NAMELB', KNMLB, NELBLK * MXSTLN)
      ...
      IF (NVAREL .GT. 0) THEN
         CALL MDFIND ('ISEVOK', KIEVOK, NELBLK * NVAREL)
      END IF
      if (iszoom .or. (nelblk .ne. nelbo)) then
