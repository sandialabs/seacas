...
C=======================================================================
C $Id: savcmd.f,v 1.1 2008/10/31 05:04:01 gdsjaar Exp $
C $Log: savcmd.f,v $
...
C
C Revision 1.4  1997/09/08 17:18:53  gdsjaar
...
C   --   Uses NVARHI, NVARGL, NVARNP, NVAREL of /DBNUMS/
      ...
      INTEGER INTYP(*)
      CHARACTER*(*) CFIELD(*)
      CHARACTER*(MXSTLN) NAMES(*), NAME

      LOGICAL FFEXST
c      CHARACTER*8 NAME, WORD
...
c         IX = LOCSTR (NAME, NVARHI+NVARGL+NVARNP+NVAREL, NAMES)
         IF (IX .GT. 0) THEN
...
c         IF ((TYPABR .EQ. 'H') .OR. (TYPABR .EQ. 'A')) THEN
c            IF (IVAR .EQ. 0) THEN
c               CALL DBVIX ('H', 1, IHV)
c               CALL ADDVAR (NAMES(IHV), NVARHI, 'H', 1,
c     &            NINP, ILHS)
c            ELSE
c               CALL ADDVAR (NAME, 1, 'H', 1, NINP, ILHS)
c            END IF
c         END IF
