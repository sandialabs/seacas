...
C=======================================================================
      PROGRAM ALGEBRA2
C=======================================================================
C $Id: algebra2.f,v 1.2 2009/03/25 14:35:16 gdsjaar Exp $
C
...
C     Set error messages to verbose
C      call exopts(EXABRT+EXVRBS, IERR)
...
C     Open the input database; Exit on error
      CALL EXNAME(NDBIN, FILNAM,LFIL)
      ndbin = exopen(filnam(:lfil), EXREAD, cpuws, iows,
     &       vers, ierr)
      IF (IERR .NE. 0) THEN
        CALL PRTERR ('FATAL', 'Database does not exist')
        call screrr (11, filnam, lfil, 'algebra2', 'algebra')
        GOTO 130
      END IF

      INOPEN = .TRUE.

C     will probably not need this flag
c      EXODUS = .FALSE.
...
C     Reserve contiguous block of dynamic memory
      ...
      CALL MCGET (2*(NELBLK * NVAREL))
C     Reserve memory for coordinate array names
      CALL MCRSRV ('NAMECO', KNACOR, MXSTLN*NDIM)
...
C                  glb_var_names, nod_var_names, elem_var_names)
     ...
     &             C(KNAMES+MXSTLN*(KNAMGV-1)),
     &             C(KNAMES+MXSTLN*(KNAMNV-1)),
     &             C(KNAMES+MXSTLN*(KNAMEV-1)))
...
C     variables (if any), then the output only history/global variables.
      CALL LNKSTO (C(KNAMES+MXSTLN*(KNAMGV-1)), NUMSTO, LTMENT, IOERR)
      IF (IOERR .EQ. 1) GO TO 130
...
C *************************************************************
      OTOPEN = .TRUE.
      CALL EXNAME(NDBOUT, FILNAM, LFIL)
...
C     processes the data for zoom mesh, and write the output database.
      ...
      CALL RWEVAL (NDBIN, NDBOUT, A, C, NPTIMS,
