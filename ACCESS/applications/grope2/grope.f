...
C      --DBNAME - the database name, needed because the database may be closed
      ...
      PARAMETER (MAXELB=256)
C      --MAXELB - the maximum number of element block type names
...
C   --Read the initial variables
     ...
     *     numnps, numess, ierr)
      ...
      CALL PRINIT ('NTIS', NOUT, DBNAME, TITLE,
...
C ... Read coordinate data
      ...
      CALL MDSTAT (NERR, MEM)
      IF (NERR .GT. 0) GOTO 100
      CALL RDCORD (NDB, NDIM, NUMNP, A(KCORD), NAMECO, ISEOF)
...
C ... Read element blocks
      ...
      CALL MCRSRV ('EBNAME', KNMEB, MXSTLN*NELBLK)
      CALL MDSTAT (NERR, MEM)
      IF (NERR .GT. 0) GOTO 100
      CALL RDELB (NDB, NELBLK,
     &   A(KIDELB), A(KNELB), A(KNLNK), A(KNATR), 
     &   A, KLINK, KATRIB, ISEOF, C(KNMLB), C(KNMEB))
      CALL MDSTAT (NERR, MEM)
      IF (NERR .GT. 0) GOTO 100
...
C ... Read sidesets
      ...
      CALL MCRSRV ('SSNAME', KNMSS,  MXSTLN*NUMESS)
      CALL MDSTAT (NERR, MEM)
      IF (NERR .GT. 0) GOTO 100
...
C   --Process commands
     ...
     &     NAMECO, C(KNMLB), C(KNMEB),
     &     C(KVNAMI+MXSTLN*(IXGV-1)), C(KVNAMI+MXSTLN*(IXNV-1)),
     &     C(KVNAMI+MXSTLN*(IXEV-1)), C(KVNAMI+MXSTLN*(IXNS-1)),
     $     C(KVNAMI+MXSTLN*(IXSS-1)),
     &     C(KVNAMO+MXSTLN*(IXGV-1)), C(KVNAMO+MXSTLN*(IXNV-1)),
     &     C(KVNAMO+MXSTLN*(IXEV-1)), C(KVNAMO+MXSTLN*(IXNS-1)),
     $     C(KVNAMO+MXSTLN*(IXSS-1)),
