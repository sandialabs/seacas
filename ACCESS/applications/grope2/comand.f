...
C   --   Uses NOUT, NCRT, NPRT, ANYPRT of /OUTFIL/
      ...
      CHARACTER*(MXSTLN) EBNAME(*), NSNAME(*), SSNAME(*)
      CHARACTER*(MXSTLN) NAMIGV(*), NAMINV(*), NAMIEV(*),
     $     NAMINS(*), NAMISS(*)
      CHARACTER*(MXSTLN) NAMOGV(*), NAMONV(*), NAMOEV(*),
     $     NAMONS(*), NAMOSS(*)
      REAL CORD(*)
      INTEGER MAPEL(*)
      INTEGER MAPNO(*)
      ...
      CHARACTER*(MXSTLN) WORD, VERB, LISTYP
      INTEGER     INTYP(MAXFLD+1)
      CHARACTER*(MXSTLN) CFIELD(MAXFLD)
      INTEGER     IFIELD(MAXFLD)
      REAL        RFIELD(MAXFLD)
      LOGICAL HELP
      LOGICAL ISON
      CHARACTER*(MXSTLN) OPT
      CHARACTER*(MXSTLN) MMNAME
      CHARACTER MMTYP
      CHARACTER*80 DUMLIN

      CHARACTER*(MXSTLN) CMDTBL(14), SELTBL(15), LISTBL(37)
 ...
 234      continue
     ...
     *        0, IDGLO, *270)
        ...
        ELSE IF ((LISTYP .EQ. 'TITLE') .OR. (LISTYP .EQ. 'VARS')) THEN
          IF (EXODUS) THEN
            CALL PRINIT ('NTSIV', NOUT, DBNAME, TITLE,
     &        NDIM, NUMNP, NUMEL, NELBLK,
     &        NUMNPS, LNPSNL, LNPSDF,
     &        NUMESS, LESSEL, LESSNL, LESSDF,
     &        NVARGL, NVARNP, NVAREL, NVARNS, NVARSS)
          ELSE
            CALL PRINIT ('NTSI', NOUT, DBNAME, TITLE,
     &        NDIM, NUMNP, NUMEL, NELBLK,
     &        NUMNPS, LNPSNL, LNPSDF,
     &        NUMESS, LESSEL, LESSNL, LESSDF,
     &        NVARGL, NVARNP, NVAREL, NVARNS, NVARSS)
     ...
     *      MAPNO, DOMAPN)
     ...
     &      NAMELB, EBNAME, NVAREL, NAMIEV, ISEVOK, A(KXLSEV),
     *      MAPNO, DOMAPN, MAPEL, DOMAPE)
     ...
     *          ia(kinvc), ia(kinvs), ia(kndmp), lisnp, NOUT, MAPNO,
     $          MAPEL, DOMAPN, DOMAPE, DOBLK, DOELE)
...
C *** Miscellaneous Commmands ***
 ...
 235     continue
      ...
      ELSE IF (VERB .EQ. 'CHECK') THEN
        ...
        CALL CKELB (NELBLK, NUMEL, NUMNP,
     &    IDELB, NUMELB, NUMLNK, NUMATR, LINK, ATRIB, IA(KCHECK),
     $       MAPNO)
