...
C   --   NAMELB - OUT - the names of the element block types
...
C ... Allocate space for variable names
      ...
      call mcrsrv('VNAMEI', kvnami, nname*mxstln)
      call mcrsrv('VNAMEO', kvnamo, nname*mxstln)
      call mcstat(nerr, nused)
      if (nerr .gt. 0) go to 240

C ... Read variable names (wrapper used to get character length correct)
      ioff = 0
      call rdnam2(ndb, c(kvnami+ioff), nvargl, 'g', ierr)
      if (ierr .ne. 0) go to 220
      call fixnam(c(kvnami+ioff), c(kvnamo+ioff), nvargl)

      ioff = ioff + (nvargl * mxstln)
      call rdnam2(ndb, c(kvnami+ioff), nvarnp, 'n', ierr)
      if (ierr .ne. 0) go to 220
      call fixnam(c(kvnami+ioff), c(kvnamo+ioff), nvarnp)
      
      ioff = ioff + (nvarnp * mxstln)
      call rdnam2(ndb, c(kvnami+ioff), nvarel, 'e', ierr)
      if (ierr .ne. 0) go to 220
      call fixnam(c(kvnami+ioff), c(kvnamo+ioff), nvarel)
      
      ioff = ioff + (nvarel * mxstln)
      call rdnam2(ndb, c(kvnami+ioff), nvarns, 'm', ierr)
      if (ierr .ne. 0) go to 220
      call fixnam(c(kvnami+ioff), c(kvnamo+ioff), nvarns)
      
      ioff = ioff + (nvarns * mxstln)
      call rdnam2(ndb, c(kvnami+ioff), nvarss, 's', ierr)
      if (ierr .ne. 0) go to 220
      call fixnam(c(kvnami+ioff), c(kvnamo+ioff), nvarss)
...
C ... Initialize in case there is error
  ...
  100 CONTINUE
      ...
      subroutine fixnam(namin, namout, nvar)
C ... Convert the variable names to uppercase and compress blanks
      include 'exodusII.inc'
      character*(mxstln) namin(*), namout(*)
