...
C     CVSREV - CVS Revision information
C     CVSDAT - CVS date information
...
C.
      CVSREV    = '$Revision: 1.1 $'
      CVSDAT    = '$Date: 2008/10/31 05:04:08 $'
      QAINFO(1) = 'exo2txt                         '
      QAINFO(2) = '                                '
      QAINFO(3) = '                                '
      QAINFO(4) = '                                '
...
C     Open the input database; Exit on error
     ...
     &       VERS, IERR)
      IF (IERR .NE. 0) THEN
         ...
         call screrr (11, filnam, lfil, 'exotxt2', 'exotxt')
         GOTO 140
      END IF

      CALL OPNFIL (NTXT, 'U', 'L', 0, IERR)
...
C     Reserve memory for coordinate array names
      CALL MCRSRV ('NAMECO', KNACOR, MXSTLN*NNDIM)

      call getxyz(ndb, c(knacor), a(kXN), A(KYN), A(KZN), IERR)
      CALL WRXYZ (NTXT, NNDIM, NUMNP, A(KXN), A(KYN), A(KZN), C(knacor))
...
C     Reserve contiguous block space for element block information
      CALL MDGET (5 * NELBLK)
...
C        Reserve Dynamic arrays needed to process information
         CALL MDGET  (4*NUMNPS + LNPSDF + 2*LNPSNL)
...
C     Reserve contiguous block of dynamic memory
      CALL MCGET (MXSTLN*(NVARGL+ NVARNP+NVAREL))
      CALL MDGET (2*(NELBLK * NVAREL))
C     Reserve memory for global, node, and element variable
      CALL MCRSRV ('NAMES', KNAMES, MXSTLN*(NVARGL+NVARNP+NVAREL))
...
C     Read the element variable truth table
     ...
     &  C(KNAMES+MXSTLN*(KNAMEV-1)), A(KIEVOK))
...
C   --Read the number of database time steps
      ...
      CALL MDGET  (NVARGL + NVARNP*NUMNP + NVAREL*NUMEL)
...
C     name array which is dynamically allocated      
      SUBROUTINE GETXYZ(NDB, NAMECO, X, Y, Z, ierr)
      include 'exodusII.inc'
