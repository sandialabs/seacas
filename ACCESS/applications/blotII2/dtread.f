...
C $Id: dtread.f,v 1.1 2009/03/25 04:47:53 gdsjaar Exp $
C $Log: dtread.f,v $
...
C   --   Uses NUMNP, NUMEL, NELBLK of /DBNUMS/
      ...
      if (proces .eq. 'MASTER') then
         N = 0
         DO 100 IVAR = MAX (NNDVAR, NEDVAR), 1, -1
            IF (IDTVAR(IVAR) .NE. 0) THEN
               N = N + 1
               IX(N) = IDTVAR(IVAR)
            END IF
  100    CONTINUE
         IF (N .GT. 0) THEN
            CALL REQVAR ('MESHVAR', ISTEP, N, IX, *170)
         END IF
      end if
...
C            --Get element variable, change to face variable
  ...
  150          CONTINUE
               ...
               CALL GTMVAR (A, IDTVAR(IVAR), -1, ISTEP,
     &            LENF(NELBLK), VARFAC(1,IVAR))
            end if
         END IF
  160 CONTINUE
