...
C $Id: msmemy.f,v 1.1 2009/03/25 04:47:53 gdsjaar Exp $
C $Log: msmemy.f,v $
...
C      --Reserve memory (most will be expanded when needed)
         ...
         if (proces .ne. 'MASTER') then
            ...
            CALL MDRSRV ('XNM', KXN, NUMNPF)
            CALL MDRSRV ('YNM', KYN, NUMNPF)
            IF (IS3DIM) THEN
               CALL MDRSRV ('ZNM', KZN, NUMNPF)
            ELSE
               KZN = 1
            END IF
         end if
...
C   --Find memory
      ...
      if (proces .ne. 'MASTER') then
         ...
         CALL MDFIND ('XNM', KXN, IDUM)
         CALL MDFIND ('YNM', KYN, IDUM)
         IF (IS3DIM) THEN
            CALL MDFIND ('ZNM', KZN, IDUM)
         ELSE
            KZN = 1
         END IF
      end if
