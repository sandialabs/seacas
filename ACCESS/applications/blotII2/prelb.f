...
C $Id: prelb.f,v 1.1 2009/03/25 04:47:54 gdsjaar Exp $
C $Log: prelb.f,v $
...
C assertion.
C
C Revision 1.3  1994/06/13 14:52:02  gdsjaar
C Determined largest variable name and use that for output so names come
C out in columns with minimal whitespace. Still need to determine number
C of columns dynamically.
C
C Revision 1.2  1994/06/07  14:45:17  gdsjaar
C Changed name size from 8 to *
C
c Revision 1.1  1994/04/07  20:07:24  gdsjaar
c Initial checkin of ACCESS/graphics/blotII2
c
CRevision 1.3  1992/05/22  22:37:27  gdsjaar
CModified to handle more than 100,000 nodes/elements in printouts
C
CRevision 1.2  1990/12/14  08:54:56  gdsjaar
...
10010  FORMAT ('(', I8, '..', I8, ')')
     ...
     &            NUME, STRB(:LSTRB), NUMLNK(IELB), NUMATR(IELB)
            ...
            IF (DONAM) THEN
               IF (NOUT .GT. 0) THEN
                  WRITE (NOUT, 10040) NAMELB(IELB)
               ELSE
                  WRITE (*, 10040) NAMELB(IELB)
               END IF
            END IF
...
10030  FORMAT (1X, 'Block', I5, 1X, A, ':',
     &   I6, ' elements', 1X, A,
     &   I4, '-node', I4, ' attributes')
10040  FORMAT (4X, 'Element block type = "', A, '"')
