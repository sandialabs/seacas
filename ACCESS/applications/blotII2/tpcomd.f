...
C $Id: tpcomd.f,v 1.1 2009/03/25 04:47:54 gdsjaar Exp $
C=======================================================================
...
C         --Input the variable line
     ...
     &               LPROM, *140)
C                  CALL FREFLD (0, 0, 'X VARIABLE> ', MAXFLD,
C     &               IOSTAT, NUMFLD, INTYP, CFIELD, IFIELD, RFIELD)
               ELSE
                  PROMPT = 'Y VARIABLE> '
                  LPROM = LENSTR (PROMPT)
                  CALL GETINS ('parse', MAXFLD, NUMFLD, INTYP,
     &               CFIELD, IFIELD, RFIELD, ' ', IOSTAT, PROMPT,
     &               LPROM, *140)
C                  CALL FREFLD (0, 0, 'Y VARIABLE> ', MAXFLD,
C     &               IOSTAT, NUMFLD, INTYP, CFIELD, IFIELD, RFIELD)
               END IF
  140          CONTINUE
