...
C $Id: msstep.f,v 1.1 2009/03/25 04:47:53 gdsjaar Exp $
C $Log: msstep.f,v $
...
C at this time.
C
C Revision 1.1  1994/04/07 20:05:59  gdsjaar
C Initial checkin of ACCESS/graphics/blotII2
C
c Revision 1.2  1990/12/14  08:54:08  gdsjaar
...
C   --Compute deformed nodal coordinates, if needed
      ...
      IF (NEWNPD .AND. ANYDEF) THEN
         ...
         if (proces .eq. 'MASTER') then
            IX(1) = IXDEF
            IX(2) = IYDEF
            IF (IS3DIM) IX(3) = IZDEF
            CALL REQVAR ('MESHVAR', ISTEP, NDIM, IX, *100)
         end if
