...
C $Id: scaele.f,v 1.1 2009/03/25 04:47:54 gdsjaar Exp $
C $Log: scaele.f,v $
...
C=======================================================================
     ...
     &   LENE, ISEVOK, NALVAR, ALIVE, VAR, WHOTIM, XE, YE, ZE,
...
C   --   NALVAR - IN - the element birth/death variable
...
C   --Transfer birth/death variable to random file (for efficiency)
      ...
      IF ((NALVAR .GT. 0) .AND. (MXSTEP .GT. 0)) THEN
         MXSTEP = 0
         DO 100 ISTEP = 1, NSTEPS
            IF (WHOTIM(ISTEP)) MXSTEP = ISTEP
  100    CONTINUE
         CALL GETVAR (A, NALVAR, -1, -MXSTEP, NUMEL, VAR)
      END IF
...
C         --Order to optimize read from a sequential file
            IF (NALVAR .LT. IVAR) THEN
               CALL GETALV (A, NALVAR, ISTEP, LENE, ISEVOK,
     &            ALIVE, ALIVE)
