...
C $Id: grinit.f,v 1.1 2009/03/25 04:47:53 gdsjaar Exp $
C $Log: grinit.f,v $
...
C   --Routines Called:
C   --   EXNAME - (SUPES) Get device name
...
C   --Get graphic output devices ready
      ...
      IF (ION .EQ. 1) THEN
         DO 110 IDEV = 1, 2
            ...
            DEVOK(IDEV) = (ION .EQ. 1)
            IF (DEVOK(IDEV)) THEN
               CALL EXNAME (-IDEV, DEVNAM(IDEV), IDUM)
               IF (DEVNAM(IDEV) .EQ. ' ' .or. idum .eq. 0)
     &              DEVNAM(IDEV) = '***'
            ELSE
               DEVNAM(IDEV) = ' '
            END IF
  110    CONTINUE
      ELSE
         DO 120 IDEV = 1, 2
            CALL EXNAME (-IDEV, DEVNAM(IDEV), IDUM)
            DEVOK(IDEV) = (DEVNAM(IDEV) .NE. ' ' .and. idum .ne. 0)
  120    CONTINUE
