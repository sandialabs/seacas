...
C $Id: getins.f,v 1.1 2009/03/25 04:47:53 gdsjaar Exp $
C $Log: getins.f,v $
...
C assertion.
C
C Revision 1.6  2008/05/05 19:42:43  gdsjaar
C Fix to work with the new command-line editting capability
C
C Revision 1.5  2007/06/05 20:43:59  gdsjaar
C Increase length of character variable used to store input filename in
C getins.f.  Was 40, increased to 256.
C
C Revision 1.4  2006/01/24 20:02:44  gdsjaar
C Fixed undefined iostat in getins.
C
C Revision 1.3  1998/07/15 14:44:17  gdsjaar
...
C
C Revision 1.1  1994/04/07 20:01:40  gdsjaar
C Initial checkin of ACCESS/graphics/blotII2
C
c Revision 1.5  1991/07/01  18:57:14  gdsjaar
c Added code to lowercase CMDFILE filename since
c majority of Unix filenames are lowercase - FREFLD automatically uppercases
c names, so we guess that it was all lowercase, also checks uppercase
c
c Revision 1.4  1991/06/25  16:08:29  gdsjaar
c Fixed calls to prterr -- too many parameters
c
c Revision 1.3  1991/06/19  18:42:35  gdsjaar
c Fixed up call to prterr, incorrect number of args
c
c Revision 1.2  1990/12/14  08:51:07  gdsjaar
...
c
            if ( batch() ) then
               call filhnd ( batin, name(1), .TRUE., ecode,
     &            'o', 'f', 's', 0, *150)
               stkpnt = stkpnt + 1       
...
c    set logical unit where instructions are to be read from
               ...
               call filhnd ( batin, name(1), .FALSE., ecode,
     &            'o', 'f', 's', 0, *150)
               if ( ecode ) then       
...
c         write (45,*) 'nfield = ',nfield
c         do 938 i = 1,nfield
c            write(45,*) 'i= ',i,' cval2(i)= ',cval2(i)
c 938     continue
      else
...
c
c           print statements for testing purposes
c
c      write ( *, * )
c      write ( *, * ) ' EXITING GETINS '
c     do 1000 i = 1,stkpnt
c        print *, 'file =', name( i ),'recred = ',recred(i)
c1000 continue

c      write (*,*) 'iostat on return = ',ios
      return
