...
C $Id: rcvxyz.f,v 1.1 2009/03/25 04:47:54 gdsjaar Exp $
C $Log: rcvxyz.f,v $
...
C assertion.
C
C Revision 1.1  1994/04/07 20:09:42  gdsjaar
C Initial checkin of ACCESS/graphics/blotII2
C
c Revision 1.2  1990/12/14  08:56:16  gdsjaar
c Added RCS Id and Log to all files
c
C=======================================================================
      SUBROUTINE RCVXYZ (OPTION, NDIM, NUMNP, XN, YN, ZN, *)
C=======================================================================

C   --*** RCVXYZ *** (BLOT) Receive database coordinates
C   --   Written by Amy Gilkey - revised 05/09/88
C   --
C   --RCVXYZ receives the coordinate array as a message.
C   --
C   --Parameters:
...
C   --   * - return statement if message error
      ...
      IF ((NDIM .LE. 0) .OR. (NUMNP .LE. 0)) RETURN

      TAGEXP = 'COORDS'
      LEXP = NUMNP
      IERR = GETRR (TAG, XN, LEXP, L)
      CALL CHKGET (IERR, TAG, TAGEXP, L, LEXP, ISOK)
      IF (.NOT. ISOK) RETURN 1

      TAGEXP = 'COORDS'
      LEXP = NUMNP
      IERR = GETRR (TAG, YN, LEXP, L)
      CALL CHKGET (IERR, TAG, TAGEXP, L, LEXP, ISOK)
      IF (.NOT. ISOK) RETURN 1
