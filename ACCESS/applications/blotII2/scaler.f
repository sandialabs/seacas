...
C $Id: scaler.f,v 1.1 2009/03/25 04:47:54 gdsjaar Exp $
C $Log: scaler.f,v $
...
C         --Calculate min/max for all history variables
     ...
     &            A(KPACK), *100)
               ...
               CALL MDSTAT (NERR, MEM)
               IF (NERR .GT. 0) GOTO 100
               CALL REQSCA ('*', IVAR, 0, .FALSE., 0, IDUM,
     &            A(KPACK), *100)
               CALL RCVSCA ('*', IVAR, 0,
     &            A(KVALHN+IX), IDUM, RDUM, A(KISTHN+IX),
     &            A(KVALHX+IX), IDUM, RDUM, A(KISTHX+IX),
...
C         --Calculate min/max for element variable by element block
     ...
     &            A(KISTEX+IX))
            ...
            if (proces .eq. 'SLAVE') then
               CALL MDRSRV ('SCR', KPACK, 2*6*(1+NELBLK))
               CALL MDSTAT (NERR, MEM)
               IF (NERR .GT. 0) GOTO 100
               CALL SNDSCA ('*', IVAR, NELBLK,
     &            A(KVALEN+IX), A(KNUMEN+IX), A(KXYZEN+IXX),
     &            A(KISTEN+IX),
     &            A(KVALEX+IX), A(KNUMEX+IX), A(KXYZEX+IXX),
     &            A(KISTEX+IX),
     &            A(KPACK), *100)
               ...
               CALL MDSTAT (NERR, MEM)
               IF (NERR .GT. 0) GOTO 100
               CALL REQSCA ('*', IVAR, NALVAR, USESEL, NELBLK, IELBST,
     &            A(KPACK), *100)
               CALL RCVSCA ('*', IVAR, NELBLK,
     &            A(KVALEN+IX), A(KNUMEN+IX), A(KXYZEN+IXX),
     &            A(KISTEN+IX),
     &            A(KVALEX+IX), A(KNUMEX+IX), A(KXYZEX+IXX),
     &            A(KISTEX+IX),
