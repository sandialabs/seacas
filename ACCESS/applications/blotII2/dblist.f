...
C $Id: dblist.f,v 1.1 2009/03/25 04:47:53 gdsjaar Exp $
C $Log: dblist.f,v $
...
C assertion.
C
C Revision 1.4  2009/01/22 15:51:01  gdsjaar
C Added minor support for nodeset and sideset variables.
C
C It can print the count and the names, but that is all
C at this time.
C
C Revision 1.3  1998/07/07 14:05:50  gdsjaar
C For exodusII files, it is not always possible to get the filename
C using an 'inquire'. Modified the prinit routine to take filename as
C argument instead of doing inquire.
C
C Revision 1.2  1994/06/13 17:04:59  gdsjaar
C Removed hash-mark which messed up make depend on Cray
C
c Revision 1.1  1994/04/07  19:58:49  gdsjaar
c Initial checkin of ACCESS/graphics/blotII2
c
c Revision 1.2  1990/12/14  08:49:11  gdsjaar
...
C   --   Uses NOUT, NCRT, NPRT, ANYPRT of /OUTFIL/
      ...
      CHARACTER*(MXSTLN) WORD, LISTYP
      CHARACTER*(MXSTLN) DUMLIN
...
C   --Get the command verb
     ...
     &         ' ', ' ', ' ')
            ...
            CALL PRINIT ('N', NPRT, NDB, FILNAM, TITLE,
...
C *** GENESIS Print Commands ***
     ...
     &         LISHV, LISGV, LISNV, LISEV)
      ...
      IF ((LISTYP .EQ. 'TITLE') .OR. (LISTYP .EQ. 'VARS')) THEN
         ...
         IF (EXODUS) THEN
            CALL PRINIT ('NTSIV', NOUT, NDB, FILNAM, TITLE,
     &         NDIM, NUMNP, NUMEL, NELBLK,
     &         NUMNPS, LNPSNL, LNPSDF, NUMESS, LESSEL, LESSNL,
     &         NVARGL, NVARNP, NVAREL, NVARNS, NVARSS)
         ELSE
            CALL PRINIT ('NTSI', NOUT, NDB, FILNAM, TITLE,
     &         NDIM, NUMNP, NUMEL, NELBLK,
     &         NUMNPS, LNPSNL, LNPSDF, NUMESS, LESSEL, LESSNL,
     &         NVARGL, NVARNP, NVAREL, NVARNS, NVARSS)
         END IF
      ...
      ELSE IF (LISTYP .EQ. 'MAP') THEN
         ...
         if (proces .eq. 'MASTER') then
            IF (N .LT. NUMEL) THEN
               ...
               IF (NERR .GT. 0) GOTO 150
               CALL REQOPT ('MAP', *150)
               CALL RCVMAP ('*', NUMEL, A(KMAPEL), *150)
            END IF
         end if
     ...
     &      .OR. (LISTYP .EQ. 'CONNECTI')) THEN
         ...
         ELSE IF (LISTYP .EQ. 'ATTRIBUT') THEN
            ...
            IF (NERR .GT. 0) GOTO 150
            if (proces .eq. 'MASTER') then
               CALL CNTLNK (NELBLK, LENE, A(KNATR), NATR, IDUM)
               IF (N .LT. NATR) THEN
                  CALL MDLONG ('ATRIB', KATRIB, NATR)
                  CALL MDSTAT (NERR, MEM)
                  IF (NERR .GT. 0) GOTO 150
                  CALL REQOPT ('ATTRIBUT', *150)
                  CALL RCVELB ('A', NELBLK,
     &               IDELB, LENE, IDUM, IDUM, A(KNATR), A, IDUM, KATRIB,
     &               IDUM, *150)
               END IF
            end if
         END IF
  ...
  110          CONTINUE
     ...
     &      NAMELB, NVAREL, NAMES(IXEV), ISEVOK, A(KXLSEV))
         ...
         ELSE IF (LISTYP .EQ. 'NNODES') THEN
            ...
            if (proces .eq. 'MASTER') then
               IF (N .LT. LNPSNL) THEN
                  ...
                  CALL MDSTAT (NERR, MEM)
                  IF (NERR .GT. 0) GOTO 150
                  CALL REQOPT ('NNODES', *150)
                  CALL RCVNPS ('N', NUMNPS, LNPSNL,
     &               IDUM, IDUM, A(KIXNNS), A(KLTNNS), RDUM, IDUM, *150)
               END IF
            end if
         ELSE IF (LISTYP .EQ. 'NFACTORS') THEN
            ...
            if (proces .eq. 'MASTER') then
               IF (N .LT. LNPSNL) THEN
                  CALL MDLONG ('FACNPS', KFACNS, LNPSNL)
                  CALL MDSTAT (NERR, MEM)
                  IF (NERR .GT. 0) GOTO 150
                  CALL REQOPT ('NFACTORS', *150)
                  CALL RCVNPS ('F', NUMNPS, LNPSNL,
     &               IDUM, IDUM, A(KIXNNS), IDUM, A(KFACNS), IDUM, *150)
               END IF
            end if
         END IF
     ...
     &               IDUM, A(KLTNSS), RDUM, IDUM, *150)
         ...
         ELSE IF (LISTYP .EQ. 'SFACTORS') THEN
            ...
            if (proces .eq. 'MASTER') then
               IF (N .LT. LESSNL) THEN
                  CALL MDLONG ('FACESS', KFACSS, LESSNL)
                  CALL MDSTAT (NERR, MEM)
                  IF (NERR .GT. 0) GOTO 150
                  CALL REQOPT ('SFACTORS', *150)
                  CALL RCVESS ('F', NUMESS, LESSEL, LESSNL,
     &               IDUM, IDUM, IDUM, IDUM, A(KIXNSS),
     &               IDUM, IDUM, A(KFACSS), IDUM, *150)
               END IF
            end if
         END IF
