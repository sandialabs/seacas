...
C     $Id: txtexo.f,v 1.1 2008/10/31 05:04:17 gdsjaar Exp $
C=======================================================================
      PROGRAM TXTEXO
C=======================================================================
...
C.
      CVSREV    = '$Revision: 1.1 $'
      CVSDAT    = '$Date: 2008/10/31 05:04:17 $'
      QAINFO(1) = 'txtexo2                         '
      QAINFO(2) = '                                '
      QAINFO(3) = '                                '
      QAINFO(4) = '                                '
...
C     --Open the database and write the initial variables
      ...
      CALL OPNFIL (NTXT, 'I', 'F', 0, IERR)
      IF (IERR .NE. 0) THEN
         CALL PRTERR ('FATAL', 'Text file does not exist')
         GOTO 140
      END IF
      EXODUS = .FALSE.

      FILOUT = ' '
      CALL EXNAME(NDB, FILOUT, LFIL)
      CMPSIZ = 0
      IOWS   = iowdsz()
      ndb = excre(filout(:lfil), EXCLOB, CMPSIZ, IOWS, IERR)
      if (ierr .lt. 0) then
...
C     --Read the initial variables
     ...
     &     NUMNPS, LNPSNL, LNPSDF, NUMESS, LESSEL, LESSNL, LESSDF, *140)
...
C     --Read the coordinates
      ...
      CALL RDXYZ (NTXT, NDIM, NUMNP, A(KXN), A(KYN), A(KZN), NAMECO,
     &  *140)
      call expcon (ndb, nameco, ierr)
      call expcor (ndb, a(kxn), a(kyn), a(kzn), ierr)
