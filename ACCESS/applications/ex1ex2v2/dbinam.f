...
C
CRevision 1.2  1998/09/16 18:06:45  gdsjaar
CChanged certain A(...) references to IA(...). This results in cleaner
Ccompile and also fixed core dump on Dec Alpha with g77.
C
...
C   --Read the element block variable truth table
      ...
      IF ((OPTION .EQ. '*') .OR. (INDEX (OPTION, 'T') .GT. 0)) THEN
         ...
         CALL DBINM1 (NDB, OPTION, NELBLK, NVAREL,
     *     IA(KIEVOK), IA(KIEVOK), IERR, MAX(NELBLK,1), *210)
...
C      --Call DBIV1
     ...
     &      NVARHI, NVARGL, NVARNP, NVAREL, IA(KIEVOK))
      ...
      ELSE
         CALL DBINM1 (NDB, OPTION, NELBLK, NVAREL, IDUM, IDUM,
