...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/output.c,v 1.3 2006/03/13 22:25:36 gdsjaar Exp $";
#endif
...
static void _O_start_standout(void)
{
    if (_O_need_init) {
	_O_st_init();
    }
...
static void _O_end_standout(void)
{
    if (_O_need_init) {
	_O_st_init();
    }
...
static void _O_pchars(char *line, int start, int end)
{
    int cnt;
...
*/
static int _O_con_line(int numb, int flags, int filenum)
{
    if (flags & U_TOKENS) {
	return (numb + 1);
    } else {
...
static char *_O_convert(char *ptr)
{
...
static char *_O_get_text(int file, int index, int flags)
{
...
static void _O_do_lines(int start, int end, int file)
{
    int cnt;
    int lastline = -1;
    int nextline;
...
void O_output(E_edit start, int flags)
{
	...
	if (_O_DEL == type || _O_CHA == type) {
		...
		for (cnt = first1; cnt <= last1; cnt++) {
		    (void) printf("< %s", _O_get_text(0, cnt, flags));
		}
	    } else {
	...
	if (_O_APP == type || _O_CHA == type) {
		...
		for (cnt = first2; cnt <= last2; cnt++) {
		    (void) printf("> %s", _O_get_text(1, cnt, flags));
		}
	    } else {
