...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/tol.c,v 1.2 2004/10/29 14:00:41 gdsjaar Exp $";
#endif
...
static void _T_tolclear(T_tol * addr)
{
    *addr = _T_null;
}
...
*/
static void _T_settol(T_tol * toladdr, char *str)
{
    char typechar;
    while ('\0' != *str) {
	/*
...
static char *_T_nextspec(char *ptr)
{
    /*
...
*/
static char *_T_getspec(char *from)
{
...
**			default tolerance
*/
T_tol T_picktol(T_tol p1, T_tol p2)
{
    if (!(T_isnull(p1)))
	return (p1);
...
void _T_appendtols(T_tol * to, T_tol from)
{
	...
	*to = from;
	 ...
	 */
	for (last = *to; !T_isnull(T_getnext(last));
	     last = T_getnext(last)) {
	}
	/*
...
*/
void _T_addtol(T_tol * listptr, int type, char *str)
{
    T_tol last;
	 ...
	 **     find the last element on the list
	 */
	for (last = *listptr; !T_isnull(T_getnext(last));
	     last = T_getnext(last)) {
	}
	/*
	 **     add an element on the end
	 ...
	 */
	if ((T_RELATIVE == type) &&
	    (F_floatcmp(T_getfloat(last), F_atof("2.0", USE_ALL)) > 0)) {
	    (void) sprintf(Z_err_buf,
