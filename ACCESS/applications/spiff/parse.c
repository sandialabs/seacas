...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/parse.c,v 1.2 2004/10/29 14:00:41 gdsjaar Exp $";
#endif
...
static void _P_alpha_clear(void)
{
    *_P_alpha = '\0';
}

static int _P_in_alpha(char chr)
{
    /*
    ...
    return ((int) (strchr(_P_alpha, chr) != NULL));
}

void P_addalpha(char *ptr)
{
    char buf[Z_LINELEN];
...
static void _P_initparser(void)
{
    _P_dummyline[0] = '\0';
...
static int _P_needmore(void)
{
    return (*_P_nextchr == '\0');
}

static int _P_nextline(void)
{
    /*
...
*/
static int _P_litsnarf(W_lit litptr)
{
    _P_stringsize = 0;
    /*
	...
	if (('\0' != *(W_litescape(litptr))) &&	/* escape string exists */
	    !S_wordcmp(_P_nextchr, W_litescape(litptr)) &&	/* and escape matches */
	    !S_wordcmp(_P_nextchr + strlen(W_litescape(litptr)), W_litend(litptr))) {	/* and endstring matches */
...
static int _P_bolsnarf(W_bol bolptr)
{
    /*
	...
	if ('\0' != *(W_bolescape(bolptr)) &&	/* escape string exists */
	    !S_wordcmp(_P_nextchr, W_bolescape(bolptr)) &&	/* and escape matches */
	    !S_wordcmp(_P_nextchr + strlen(W_bolescape(bolptr)), W_bolend(bolptr))) {	/* and end string matches */
...
*/
static int _P_comsnarf(W_com comptr)
{
	...
	if ('\0' != *(W_comescape(comptr)) &&	/* escape string exists */
	    !S_wordcmp(_P_nextchr, W_comescape(comptr)) &&	/* and escape matches */
	    !S_wordcmp(_P_nextchr + strlen(W_comescape(comptr)), W_comend(comptr))) {	/* and end string matches */
	    /*
	 ...
	 */
	if (W_is_nesting(comptr) && !S_wordcmp(_P_nextchr, W_comend(comptr))) {	/* end matches */
	    _P_nextchr += strlen(W_comend(comptr));
	    depth++;
	    continue;
	}
...
static void _P_do_parse(void)
{
	...
	if (!L_getcount(_P_fnumb, L_gettlmax(_P_fnumb))) {
	    L_settlindex(_P_fnumb,
			 L_gettlmax(_P_fnumb), L_getclmax(_P_fnumb));
	    /*
	     **     and the pointer from the token line to the 
	     **     first  token on the line
	     */
	    L_setindex(_P_fnumb,
		       L_gettlmax(_P_fnumb), K_gettmax(_P_fnumb));
	}
	...
	if ((litptr = W_islit(_P_nextchr)) != W_LITNULL) {
		 ...
		 */
		(void) strcpy(ptr, L_getcline(_P_fnumb, startline)
			      + startpos);
		/*
		...
		for (tmp = startline + 1; tmp < endline; tmp++) {
		    (void) strcat(ptr, L_getcline(_P_fnumb, tmp));
		}
		/*
		...
		(void) strncpy(ptr, L_getcline(_P_fnumb, startline)
			       + startpos, _P_stringsize);
		/*
	 ...
	 */
		...
		K_setfloat(newtoken, F_atof(K_gettext(newtoken), USE_ALL));
	 ...
	 */
	     ...
	     */
	    for (ptr = _P_nextchr; isalpha(*ptr)
		 || isdigit(*ptr)
		 || _P_in_alpha(*ptr); ptr++) {
	    }
	    K_saventext(newtoken, _P_nextchr, ptr - _P_nextchr);
...
void P_file_parse(int num, int strt, int lcnt, int flags)
		/* file number */
		/* first line to parse expressed in real line numbers */
		/* max number of lines to parse */
		/* flags for controlling the parse mode */
{
    /*
