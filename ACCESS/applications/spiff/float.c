...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/float.c,v 1.3 2004/10/29 14:00:40 gdsjaar Exp $";
#endif
...
*/
int F_isfloat(char *str, int need_decimal, int allow_sign)

			/* if non-zero, require that a decimal point be present
			   otherwise, accept strings like "123" */
			/* if non-zero, allow + or - to set the sign */
{
...
**		similar to atof()
*/
F_float F_atof(char *str, int allflag)

		/* require that exactly all the characters are used */
{
...
*/
	...
	if ('\0' != *str) {
	    (void) sprintf(Z_err_buf,
			   "didn't use up all of %s in atocf", beg);
	    Z_fatal(Z_err_buf);
	}
    }
...
*/
static
void _F_stradd(char *s1, char *s2)
{
...
*/
static void addzeros(char *ptr, int count)
{
    for (; count > 0; count--) {
	(void) strcat(ptr, "0");
    }
...
*/
F_float F_floatsub(F_float p1, F_float p2)
{
     ...
     */
    for (i = (strlen(big) - 1), borrow = 0, diff[strlen(big)] = '\0';
	 i >= 0; i--) {
	char from;
	if (borrow) {
	    if (big[i] == '0') {
...
F_float F_floatmul(F_float f1, F_float f2)
{
    ...
    R_setexp(result,
	     (((R_getexp(f1) + R_getexp(f2)) - len) + strlen(prod)));
...
*/
F_float F_floatmagadd(F_float p1, F_float p2)
{
    static F_float result;
    static int needinit = 1;
