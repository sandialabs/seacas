...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/miller.c,v 1.2 2004/10/29 14:00:41 gdsjaar Exp $";
#endif
...
E_edit G_do_miller(int m, int n, int max_d, int comflags)
{
    int max_obj = 2*(m + n);
    int lower, upper, d, k, row, col;
    E_edit new;
...
#endif
    for (row = 0; row < m && row < n && X_com(row, row, comflags) == 0;
	 ++row);
    last_d[ORIGIN] = row;
	...
	if ((d > 0) && (0 == (d % MILLER_CHATTER))) {
	    (void) sprintf(Z_err_buf, "found %d differences\n", d);
	    Z_chatter(Z_err_buf);
	}
#endif
    }
