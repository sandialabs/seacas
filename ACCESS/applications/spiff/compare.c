...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/compare.c,v 1.2 2004/10/29 14:00:40 gdsjaar Exp $";
#endif
...
static int _X_floatdiff(F_float p1, F_float p2, T_tol the_tol)
{
     ...
     */
    for (tol_tmp = the_tol; !(T_isnull(tol_tmp));
	 tol_tmp = T_getnext(tol_tmp)) {
	if ((T_IGNORE == T_gettype(tol_tmp)) ||
	    ...
	    ((T_ABSOLUTE == T_gettype(tol_tmp))
	     ...
	     && (F_getexp(p1) < F_getexp(T_getfloat(tol_tmp)) - 1)
	     && (F_getexp(p2) < F_getexp(T_getfloat(tol_tmp)) - 1))) {
	    return (0);
	}
    }
