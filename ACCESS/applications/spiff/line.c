...
#ifndef lint
static char rcsid[] =
    "$Header: /usr/sharelan/cvs/SEACAS/prepost/spiff/line.c,v 1.2 2004/10/29 14:00:41 gdsjaar Exp $";
#endif
...
static void _L_setrline(int file, int X, char *str)
{
    if (file) {
...
int L_init_file(int fnumber, char *fname)
{
	...
	if ('\n' != buf[tmplen - 1]) {
		...
		(void) sprintf(Z_err_buf,
			       ...
			       L_getrlmax(fnumber) + 1, fname, Z_LINELEN);
