XCOMM!/bin/csh -f
XCOMM $Id: plot.c,v 1.1 2008/10/31 05:19:57 gdsjaar Exp $
XCOMM This script is used to execute the plotting codes in 
XCOMM Sandia National Laboratories ACCESS system
XCOMM
...
XCOMM number.
XCOMM
XCOMM Then, all options are processed, if any files are assigned by the
XCOMM options, then those are assigned to the specified unit numbers, and
XCOMM all other options are assigned to the correct variable.
XCOMM
XCOMM Next, all required files are checked for existance, and if any files
XCOMM that are written exist, a warning message that they will be over-
XCOMM written is echoed.
XCOMM
