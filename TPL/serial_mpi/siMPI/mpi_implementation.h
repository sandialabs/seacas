 ...
 *    $RCSfile: mpi_implementation.h,v $
 *    $Author: gdsjaar $
...
extern _MPI_REQUEST_OBJECT* _MPI_REQ_LIST;
...
int _MPI_Set_Request(MPI_Request* request, void* message, int count, MPI_Datatype datatype, int send, int tag, MPI_Comm comm);
...
int _MPI_Req_Post (void* buffer, int size, MPI_Datatype type, int tag, MPI_Comm comm, int send);
