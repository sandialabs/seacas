...
/*=============================================================================================*/
/* This is checking for MPI_Status */
int _MPI_checkStatus (MPI_Status* status)
{
  if ( (status == (MPI_Status *)NULL)||(status == NULL) )
...
int _MPI_Bbuff_Insert(void *message,int count,MPI_Datatype datatype,int tag,MPI_Comm comm)
{
  ...
  for (index=0; index < _MPI_DATA_ARRAY_SIZE ; index++)
  {
    if (_MPI_DATA_BUFF[index].valid == _MPI_NOT_VALID)
    {
      ...
      _MPI_DATA_BUFF[index].size = count;
  ...
  _MPI_DATA_BUFF = (_MPI_DATA_ENTRY*) _MPI_safeRealloc(_MPI_DATA_BUFF, (_MPI_DATA_ARRAY_SIZE+PREALLOCATION_SIZE)*sizeof(_MPI_DATA_ENTRY), "Error in _MPI_Buff_Insert for reallocation");
...
int _MPI_Data_Invalid(int index) {
  if ( (index>=0)&&(index<_MPI_DATA_ARRAY_SIZE) ) {
    ...
    _MPI_DATA_BUFF[index].size = _MPI_NOT_VALID;
