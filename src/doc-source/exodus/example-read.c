...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < num_dim; i++) {
    coord_names[i] = (char *) calloc((MAX_STR_LENGTH + 1),
				     sizeof(char));
  }
...

  {
	...
	for (j = 0; j < num_attrs; j++) {
	  error = ex_get_one_attr(exoid, EX_NODAL,
				  0, j + 1, attrib);
	  free(attrib_names[j]);
	}
	free(attrib);
  ...
  if (num_elem_blk > 0) {
    ...
    for (i = 0; i < num_props; i++) {
      prop_names[i] = (char *) calloc((MAX_STR_LENGTH + 1),
				      sizeof(char));
    }
	...
	error = ex_get_prop(exoid, EX_ELEM_BLOCK, ids[j],
  ...
  for (i = 0; i < num_elem_blk; i++) {
    if (num_elem_in_block[i] > 0) {
      connect =	(int *)	calloc((num_nodes_per_elem[i] *
				num_elem_in_block[i]),
			       sizeof(int));
  ...
  for (i = 0; i < num_elem_blk; i++) {
	...
	attrib_names[j] = (char *) calloc((MAX_STR_LENGTH + 1),
					  sizeof(char));

      attrib = (float *) calloc(num_attr[i] * num_elem_in_block[i],
				sizeof(float));
      error = ex_get_attr(exoid, EX_ELEM_BLOCK, ids[i], attrib);

      if (error == 0) {
	error = ex_get_attr_names(exoid, EX_ELEM_BLOCK, ids[i],
				  attrib_names);
      }
      free(attrib);
      for (j = 0; j < num_attr[i]; j++)
  ...
  if (num_node_sets > 0) {
    ...
    for (i = 0; i < num_node_sets; i++) {
      nset_names[i] = (char *) calloc((MAX_STR_LENGTH + 1),
				      sizeof(char));
    }
    ...
    for (i = 0; i < num_node_sets; i++) {
      ...
      error = ex_get_set_param(exoid, EX_NODE_SET, ids[i],
			       &num_nodes_in_set,
			       &num_df_in_set);
      free(nset_names[i]);
      ...
      if (num_df_in_set > 0) {
	error = ex_get_set_dist_fact(exoid, EX_NODE_SET,
				     ids[i], dist_fact);
      }
	...
	int num_attrs = 0;
	error =
	  ex_get_attr_param(exoid, EX_NODE_SET, ids[i],
			    &num_attrs);
	if (num_attrs > 0) {
	  for (j = 0; j < num_attrs; j++) {
	    attrib_names[j] = (char *) calloc((MAX_STR_LENGTH + 1),
	  ...
	  error =ex_get_attr_names(exoid, EX_NODE_SET, ids[i],
				   attrib_names);

	  if (error == 0) {
	    attrib = (float *) calloc(num_nodes_in_set, sizeof(float));
	    for (j = 0; j < num_attrs; j++) {
	      error = ex_get_one_attr(exoid, EX_NODE_SET,
				      ids[i], j + 1, attrib);
	      free(attrib_names[j]);
	    }
	    free(attrib);
  ...
  if (num_side_sets > 0) {
    ...
    for (i = 0; i < num_side_sets; i++) {
      sset_names[i] = (char *) calloc((MAX_STR_LENGTH + 1),
				      sizeof(char));
    }
    ...
    for (i = 0; i < num_side_sets; i++) {
      ...
      error = ex_get_set_param(exoid, EX_SIDE_SET, ids[i],
			       &num_sides_in_set,
			       &num_df_in_set);
      free(sset_names[i]);
      ...
      if (num_df_in_set > 0) {
	error = ex_get_set_dist_fact(exoid, EX_SIDE_SET,
				     ids[i], dist_fact);
      }
	...
	error = ex_get_prop(exoid, EX_SIDE_SET, ids[j],
			    prop_names[i], &prop_value);
      }
    }
    for (i = 0; i < num_props; i++)
  ...
  for (i = 0; i < num_qa_rec; i++) {
    for (j = 0; j < 4; j++) {
      qa_record[i][j] = (char *) calloc((MAX_STR_LENGTH + 1),
					sizeof(char));
    }
  }
  ...
  if (num_nodes > 0) {
    ...
    for (i = 0; i < num_nod_vars; i++) {
      var_names[i] = (char *) calloc((MAX_STR_LENGTH + 1),
				     sizeof(char));
    }
  ...
  if (num_elem > 0) {
    ...
    error = ex_get_variable_names(exoid, EX_ELEM_BLOCK,
				  num_ele_vars, var_names);
    for (i = 0; i < num_ele_vars; i++) {
      free(var_names[i]);
    }
    ...
    if (num_ele_vars > 0) {
      ...
      error = ex_get_truth_table(exoid, EX_ELEM_BLOCK,
				 num_elem_blk, num_ele_vars,
				 truth_tab);
      free(truth_tab);
    }
  }
  ...
  if (num_node_sets > 0) {
	...
	var_names[i] =
      ...
      error = ex_get_variable_names(exoid, EX_NODE_SET,
				    num_nset_vars, var_names);
	...
	free(var_names[i]);
      ...
      if (num_nset_vars > 0) {
	truth_tab = (int *) calloc((num_node_sets * num_nset_vars),
				   sizeof(int));

	error = ex_get_truth_table(exoid, EX_NODE_SET, num_node_sets,
				    num_nset_vars, truth_tab);
	free(truth_tab);
      }
    }
  }
  ...
  if (num_side_sets > 0) {
	...
	var_names[i] = (char *) calloc((MAX_STR_LENGTH + 1),
      ...
      error = ex_get_variable_names(exoid, EX_SIDE_SET,
				    num_sset_vars, var_names);
	...
	free(var_names[i]);
      ...
      if (num_sset_vars > 0) {
	truth_tab = (int *) calloc((num_side_sets * num_sset_vars),
				   sizeof(int));

	error = ex_get_truth_table(exoid, EX_SIDE_SET,
				   num_side_sets,
				   num_sset_vars, truth_tab);
	free(truth_tab);
      }
    }
  }
  ...
  error = ex_get_var(exoid, time_step, EX_GLOBAL, 1, 0,
		     num_glo_vars, var_values);
  free(var_values);
  ...
  error = ex_get_var_time(exoid, EX_GLOBAL, var_index, 1,
  ...
  if (num_nodes > 0) {
    ...
    error = ex_get_var(exoid, time_step, EX_NODAL, var_index, 1,
    ...
    node_num = 1;
    error = ex_get_var_time(exoid, EX_NODAL, var_index,
			    node_num, beg_time, end_time, var_values);
    free(var_values);
  }
  ...
  if (num_elem_blk > 0) {
    ...
    for (i = 0; i < num_elem_blk; i++) {
      if (num_elem_in_block[i] > 0) {
	var_values = (float *) calloc(num_elem_in_block[i],
				      sizeof(float));

	error = ex_get_var(exoid, time_step, EX_ELEM_BLOCK,
			   var_index, ids[i], num_elem_in_block[i],
			   var_values);
  ...
  if (num_ele_vars > 0) {
    ...
    var_index = 2;
    elem_num = 2;
    error = ex_get_var_time(exoid, EX_ELEM_BLOCK, var_index,
			    elem_num, beg_time, end_time, var_values);
  ...
  if (num_sset_vars > 0) {
    for (i = 0; i < num_side_sets; i++) {
      var_values = (float *) calloc(ss_specs.num_entries_per_set[i],
				    sizeof(float));

      error = ex_get_var(exoid, time_step, EX_SIDE_SET,
			 var_index,
			 ss_specs.sets_ids[i],
  ...
  if (num_nset_vars > 0) {
    for (i = 0; i < num_node_sets; i++) {
      var_values = (float *) calloc(ns_specs.num_entries_per_set[i],
				    sizeof(float));

      error = ex_get_var(exoid, time_step, EX_NODE_SET,
			 var_index,
			 ns_specs.sets_ids[i],
			 ns_specs.num_entries_per_set[i],
			 var_values);
