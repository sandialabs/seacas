...
C    
C $Id: ewvnam.f,v 1.3 2008/03/14 13:50:49 gdsjaar Exp $
C************************************************************************
      subroutine ewvnam (id, nvargl, nvarnp, nvarel, glob,
     &                   node, elem)
C************************************************************************
C
...
C     read names of global variables
      if (nvargl .gt. 0)
     &   call expvan (id, 'G', nvargl, glob, ierr)

C     read names of nodal variables
      if (nvarnp .gt. 0)
     &   call expvan (id, 'N', nvarnp, node, ierr)

C     read names of element variables
      if (nvarel .gt. 0)
     &   call expvan (id, 'E', nvarel, elem, ierr)
